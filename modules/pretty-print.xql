...
(:~
 ...
 :)
declare function pretty:namespace-decls($elem as element(), $namespaces as xs:string*) as element()? {
...
declare function pretty:pretty-print($node as node()) {
    pretty:pretty-print($node, ())
};

declare function pretty:pretty-print($node as node(), $namespaces as xs:string*) as item() {
	typeswitch ($node)
