...
r,a.getActiveDocument()&&(localStorage["eXide.activeTab"]=a.getActiveDocument().path),a.saveState(),e.saveState())},getLogin:function(a){$.ajax({url:"login",dataType:"json",success:function(b){b&&b.user?(i.login=b,$("#user").text("Logged in as "+i.login.user+". "),a&&a(i.login.user)):(i.login=null,a&&a(null))},error:function(){i.login=null;$("#user").text("Login");a&&a(null)}})},enforceLogin:function(){i.requireLogin(function(){(!i.login||"guest"===i.login.user)&&i.enforceLogin()})},$checkLogin:function(){if(i.login)return!0;
c.error("Warning: you are not logged in.");return!1},requireLogin:function(a){i.login?a():($("#login-dialog").dialog("option","close",function(){i.login?a():c.error("Warning: you are not logged in!")}),$("#login-dialog").dialog("open"))},showPreferences:function(){g.show()},getPreference:function(a){return g.get(a)},startDebug:function(){var b=$("#debug span.ui-icon");b.hasClass("ui-icon-stop")?(b.removeClass("ui-icon-stop"),b.addClass("ui-icon-play")):(b.removeClass("ui-icon-play"),b.addClass("ui-icon-stop"));
a.exec("debug");$.log("start debugging click")},stepOver:function(){a.exec("stepOver")},stepInto:function(){a.exec("stepInto")},setTheme:function(a){$("#outline-body,#directory-body,#results-body ").removeClass().addClass(a.cssClass)},updateStatus:function(a){$("#syntax").val(a.getSyntax());$("#status .path").text(c.normalizePath(a.getPath()));!a.isNew()&&("xquery"==a.getSyntax()||"html"==a.getSyntax()||"xml"==a.getSyntax())?($("#status a").attr("href",a.getExternalLink()),$("#status a").css("visibility",
"visible")):$("#status a").css("visibility","hidden")},showResultsPanel:function(){b.show(r,!0);i.resize(!0)},toggleResultsPanel:function(){b.toggle(r);i.resize(!0)},prepareResultsPanel:function(a,b){var c=document.getElementById("results-iframe");$("#results-body").parent().children(":not(.resize-handle)").detach().appendTo(".panel-"+a);"html"==$("#serialization-mode").val()?($(c).show(),$("#serialization-mode").attr("disabled","disabled").val("html"),b&&i.runQuery()):$("#results-iframe").hide()},
switchResultsPanel:function(){var a="south"===r?"east":"south";i.prepareResultsPanel(a,!0);b.hide(r);r=a;"south"===r?$(".layout-switcher").attr("src","resources/images/layouts_split.png"):$(".layout-switcher").attr("src","resources/images/layouts_split_vertical.png");i.showResultsPanel()},initStatus:function(a){$("#splash-status").text(a)},git:{branch:function(a){console.info("git.branch");i.login.isAdmin&&$.ajax({type:"GET",url:"modules/git.xql",data:{target:a.root,"git-command":"branch"},dataType:"json",
success:function(b){b=b.stdout?$.isArray(b.stdout.line)?b.stdout.line:[b.stdout.line]:[];a.gitBranch=$.map(b,function(b,c){var d=b.split(" ").pop();/^\*/.test(b)&&(a.gitCurrentBranch=d,a.gitCurrentBranchIndex=c);return d});$("#toolbar-current-branch").text(a.gitCurrentBranch);$("#menu-git-active").text(a.gitCurrentBranch);$("#menu-git-working-dir").text(a.workingDir)},error:s})},command:function(b,c,d,e){i.login.isAdmin&&$.ajax({type:"GET",url:"modules/git.xql",data:{target:b.root,"git-command":c,
"git-option":d},dataType:"json",success:function(b){"function"==typeof e&&e(b);a.updateStatus("");a.clearErrors();i.showResultsPanel();q=n=1;var c=document.getElementById("results-iframe");$(c).show();c.contentWindow.document.open("text/html","replace");c.contentWindow.document.write(JSON.stringify(b));c.contentWindow.document.close()},error:s})}},findFiles:function(){var b=a.getActiveDocument();d.findProject(b.getBasePath(),function(c){eXide.find.Files.open(b,c,function(b){a.updateStatus("");a.clearErrors();
q=n=1;var c=document.getElementById("results-iframe");$(c).show();eXide.app.showResultsPanel();c.contentWindow.document.open("text/html","replace");c.contentWindow.document.write("<html><body><p>Searching ...</p></body></html>");c.contentWindow.document.close();c.src="modules/search.xql?"+b})})},liveReload:function(){var b=a.getActiveDocument();d.findProject(b.getBasePath(),function(a){if(null!=a&&($.log("live reload: %s %s",a.liveReload,a.abbrev),a.liveReload)){var b=a.url.replace(/\/{2,}/,"/");
...
b={user:b,password:c};$('#login-form input[name="duration"]').is(":checked")&&(b.duration="P14D");$.ajax({url:"login",data:b,dataType:"json",success:function(b){if(b.user){i.login=b;$.log("Logged in as %o. Is dba: %s",b,i.login.isAdmin);$("#login-dialog").dialog("close");$("#user").text("Logged in as "+i.login.user+". ");a.focus()}else{$("#login-error").text("Login failed.");$("#login-dialog input:first").focus()}},error:function(a,b,c){$("#login-error").text("Login failed. "+c);$("#login-dialog input:first").focus()}})},
