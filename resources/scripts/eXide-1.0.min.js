...
Constr.prototype.closeTag=function(f,d,c){var f="xmldb:exist://"+f.getBasePath(),g=this;$.ajax({type:"POST",url:"modules/compile.xql",data:{q:d,base:f},dataType:"json",success:function(d){d.result=="fail"&&(d=a(d.error),d.line<=c&&(d=/constructor:\s(.*)$/.exec(d.msg),d.length>0&&g.editor.insert(d[1]+">")))},error:function(){}})};Constr.prototype.validate=function(a,d,c){$.log("Running validation...");var g=this,b="xmldb:exist://"+a.getBasePath();$.ajax({type:"POST",url:"modules/compile.xql",data:{q:d,
base:b},dataType:"json",success:function(d){g.compileError(d,a);c.call(this,!0)},error:function(d,a){c.call(this,!0);$.log("Compile error: %s - %s",a,d.responseText)}})};Constr.prototype.compileError=function(f,d){if(f.result=="fail"){var c=a(f.error),b=[{row:c.line,text:c.msg,type:"error"}];this.parent.updateStatus(c.msg,d.getPath()+"#"+c.line);d.getSession().setAnnotations(b)}else this.parent.clearErrors(),this.parent.updateStatus("")};Constr.prototype.autocomplete=function(a){require("pilot/lang");
for(var d=require("ace/range").Range,c=this.editor.getSelection(),b=a.getSession(),c=c.getSelectionLead(),b=b.getDisplayLine(c.row),e=c.column-1,i=c.column;e>=0;){var j=b.substring(e,i);if(j.match(/^\$[\w:\-_\.]+$/))break;if(!j.match(/^[\w:\-_\.]+$/)){e++;break}e--}b=b.substring(e,i);$.log("completing token: %s",b);d=new d(c.row,e,c.row,i);c=this.editor.renderer.textToScreenCoordinates(c.row,c.column);e=this.parent.getHeight();if(c.pageY+150>e)c.pageY=e-150,$.log("window height: %i, pageY: %i",e,
c.pageY);$("#autocomplete-box").css({left:c.pageX+"px",top:c.pageY+10+"px"});$("#autocomplete-help").css({left:c.pageX+324+"px",top:c.pageY+10+"px"});b.length==0?this.templateLookup(a,b,d,!0):this.functionLookup(a,b,d,!0);return!0};Constr.prototype.$localVars=function(a,d){for(var c=[],b=/declare function|};/,e=/let \$[\w\:]+|for \$[\w\:]+|\$[\w\:]+\)/,i=/\$[\w\:]+/,j=RegExp("^\\"+a),k=this.editor.getSession(),l=d.start.row;l>-1;){var m=k.getDisplayLine(l),n;if(n=m.match(e))$.log("Var: %s",n[0]),
n=n[0].match(i),n[0].match(j)&&c.push({name:n[0],type:"variable"});if(m.match(b)){$.log("Stop: %s",m);break}l--}return c};Constr.prototype.functionLookup=function(a,d,c,b){var e=this;$.ajax({url:"modules/docs.xql",dataType:"text",type:"POST",data:{prefix:d},success:function(i){var i=$.parseJSON(i),j=[],k;d.substring(0,1)=="$"?(k="^\\"+d,j=e.$localVars(d,c,b)):k="^"+d;var l=RegExp(k);$.each(a.functions,function(d,a){a.name.match(l)&&j.push(a)});i&&(j=j.concat(i));i=[];for(k=0;k<j.length;k++){var m=
{label:j[k].signature?j[k].signature:j[k].name,type:j[k].type};if(j[k].help)m.tooltip=j[k].help;i.push(m)}e.$addTemplates(d,i);e.$showPopup(a,c,i)},error:function(d,a){eXide.util.error(a)}})};Constr.prototype.templateLookup=function(a,d,c){var b=[];this.$addTemplates(d,b);this.$showPopup(a,c,b)};Constr.prototype.$addTemplates=function(a,d){for(var c=this.parent.outline.getTemplates(a),b=0;b<c.length;b++)d.push({type:"template",label:"[S] "+c[b].name,tooltip:c[b].help,template:c[b].template})};Constr.prototype.$showPopup=
function(a,d,c){var b=this;eXide.util.popup($("#autocomplete-box"),$("#autocomplete-help"),c,function(c){if(c){var e=c.label;c.type=="template"?e=c.template:c.type=="function"&&(e=eXide.util.parseSignature(e));a.template=new eXide.edit.Template(b.parent,d,e,c.type);a.template.insert()}b.editor.focus()})};Constr.prototype.getFunctionAtCursor=function(a){var d=a.row,d=this.editor.getSession().getDisplayLine(d),c=a.column;do c--;while(c>=0&&d.charAt(c).match(b));c++;for(a=a.column;a<d.length&&d.charAt(a).match(b);)a++;
...
d&&this.parent.outline.gotoDefinition(a,d)};Constr.prototype.locate=function(a,d,c){switch(d){case "function":this.gotoFunction(a,c);break;default:this.gotoVarDecl(a,c)}};Constr.prototype.gotoFunction=function(a,d){$.log("Goto function %s",d);var c=this.getModuleNamespacePrefix();c!=null&&(d=d.replace(/[^:]+:/,c+":"));for(var c=RegExp("function\\s+"+d+"\\s*\\("),b=a.$session.getLength(),e=0;e<b;e++)if(a.$session.getLine(e).match(c)){this.editor.gotoLine(e+1);this.editor.focus();break}};Constr.prototype.gotoVarDecl=
function(a,d){var c=this.getModuleNamespacePrefix();c!=null&&(d=d.replace(/[^:]+:/,"$"+c+":"));$.log("Goto variable declaration %s",d);for(var c=RegExp("variable\\s+\\"+d),b=a.$session.getLength(),e=0;e<b;e++)if(a.$session.getLine(e).match(c)){this.editor.gotoLine(e+1);this.editor.focus();break}};Constr.prototype.getModuleNamespacePrefix=function(){for(var a=/^\s*module\s+namespace\s+([^=\s]+)\s*=/,d=this.parent.getActiveDocument().$session.getLength(),c=0;c<d;c++){var b=this.parent.getActiveDocument().$session.getLine(c).match(a);
if(b)return b[1]}return null};var e=/.*line:?\s(\d+)/i;return Constr}();eXide.namespace("eXide.edit.XMLModeHelper");
eXide.edit.XMLModeHelper=function(){Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.addCommand("closeTag",this.closeTag)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.closeTag=function(a,b,e){a.getBasePath();var f=this;$.ajax({type:"POST",url:"modules/validate-xml.xql",data:{xml:b},dataType:"json",success:function(a){a.status&&a.status=="invalid"&&parseInt(a.message.line)-1<=e&&(a=/element type \"([^\"]+)\"/.exec(a.message["#text"]),a.length>0&&f.editor.insert(a[1]+
">"))},error:function(){}})};Constr.prototype.validate=function(a,b,e){var f=this;$.ajax({type:"POST",url:"modules/validate-xml.xql",data:{xml:b},dataType:"json",success:function(d){f.compileError(d,a);e.call(this,!0)},error:function(a,c){e.call(this,!0);$.log("Compile error: %s - %s",c,a.responseText)}})};Constr.prototype.compileError=function(a,b){$.log("Validation returned %o",a);if(a.status&&a.status=="invalid"){var e=[{row:parseInt(a.message.line)-1,text:a.message["#text"],type:"error"}];$.log("annotation: %o",
...
"...");a={path:f.activeDoc.path,data:f.activeDoc.getText()};if(f.activeDoc.mime)a.mime=f.activeDoc.mime;$.ajax({url:"modules/store.xql",type:"POST",data:a,dataType:"json",success:function(a){a.status=="error"?b?b.apply(f.activeDoc,[a.message]):eXide.util.error(a.message):(f.activeDoc.saved=!0,f.updateTabStatus(e,f.activeDoc),c?c.apply(f.activeDoc):eXide.util.message(f.activeDoc.name+" stored."))},error:function(a){f.activeDoc.path=e;f.activeDoc.name=j;b?b.apply(f.activeDoc,a.responseText):eXide.util.error(a.responseText)}})},
...
a);this.collection=a;this.grid.invalidate();this.data.length=0;this.grid.getSelectionModel().setSelectedRanges([]);this.grid.onViewportChanged.notify()},load:function(a,b){var c=this;$.getJSON("modules/collections.xql",{root:this.collection,view:"r",start:a,end:b},function(e){for(var h=a;h<=b;h++)c.grid.invalidateRow(h);if(e&&e.items){c.data.length=e.total;for(h=0;h<e.items.length;h++)c.data[a+h]=e.items[h]}else c.data.length=0;c.grid.updateRowCount();c.grid.render();a==0&&(c.grid.setActiveCell(0,
0),c.grid.setSelectedRows([0]),c.container.find(".grid-canvas").focus())})},hasSelection:function(){var a=this.grid.getSelectionModel().getSelectedRows();return a&&a.length>0},deleteResource:function(){var a=this.grid.getSelectionModel().getSelectedRows();if(a.length!=0){for(var b=[],c=0;c<a.length;c++)b.push(this.data[a[c]].name);var e=this;eXide.util.Dialog.input("Confirm Deletion","Are you sure you want to delete the selected resources?",function(){$.log("Deleting resources %o",b);$.getJSON("modules/collections.xql",
{remove:b,root:e.collection},function(a){$.log(a.status);a.status=="fail"?eXide.util.Dialog.warning("Delete Resource Error",a.message):e.reload()})})}},reload:function(){this.update(this.collection)},addEventListener:function(a,b,c){(a=this.events[a])&&a.push({obj:b,callback:c})},$triggerEvent:function(a,b){var c=this.events[a];if(c)for(var e=0;e<c.length;e++)c[e].callback.apply(c[e].obj,b)}};return Constr}();eXide.namespace("eXide.browse.Upload");
...
eXide.browse.Browser=function(){Constr=function(a){var b=this,e=$(".eXide-browse-toolbar",a),f=document.createElement("button");f.title="Reload";f.id="eXide-browse-toolbar-reload";f.tabindex=1;var d=document.createElement("img");d.src="resources/images/arrow_refresh.png";f.appendChild(d);$(f).click(function(){b.collections.reload()});e.append(f);this.btnDeleteCollection=document.createElement("button");this.btnDeleteCollection.title="Delete Collection";this.btnDeleteCollection.id="eXide-browse-toolbar-delete-collection";
this.btnDeleteCollection.tabindex=2;d=document.createElement("img");d.src="resources/images/folder_delete.png";this.btnDeleteCollection.appendChild(d);$(this.btnDeleteCollection).click(function(a){a.preventDefault();b.collections.deleteCollection()});e.append(this.btnDeleteCollection);this.btnCreateCollection=document.createElement("button");this.btnCreateCollection.title="Create Collection";this.btnCreateCollection.id="eXide-browse-toolbar-create";this.btnCreateCollection.tabindex=3;d=document.createElement("img");
d.src="resources/images/folder_add.png";this.btnCreateCollection.appendChild(d);$(this.btnCreateCollection).click(function(a){a.preventDefault();b.collections.createCollection()});e.append(this.btnCreateCollection);this.btnUpload=document.createElement("button");this.btnUpload.title="Upload Files";this.btnUpload.id="eXide-browse-toolbar-upload";this.btnUpload.tabindex=4;d=document.createElement("img");d.src="resources/images/database_add.png";this.btnUpload.appendChild(d);$(this.btnUpload).click(function(b){b.preventDefault();
$(".eXide-browse-resources",a).hide();$(".eXide-browse-upload",a).show()});e.append(this.btnUpload);this.btnDeleteResource=document.createElement("button");this.btnDeleteResource.title="Delete Resource";this.btnDeleteResource.id="eXide-browse-toolbar-delete-resource";d=document.createElement("img");d.src="resources/images/page_delete.png";this.btnDeleteResource.appendChild(d);$(this.btnDeleteResource).click(function(a){a.preventDefault();b.resources.deleteResource()});e.append(this.btnDeleteResource);
