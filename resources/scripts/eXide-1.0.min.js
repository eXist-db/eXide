...
eXide.edit.LessModeHelper=function(){var a=require("ace/token_iterator").TokenIterator;Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.addCommand("locate",this.locate)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.documentSaved=function(a){var e=this,d=a.getText(),b="/exist/apps/"+a.getBasePath().replace(/^\/db\//,"")+"/";(new less.Parser({paths:[b],optimization:3})).parse(d,function(b,d){b?eXide.util.error("Error: "+b.message):e.saveCSS(a,d.toCSS())})};
Constr.prototype.saveCSS=function(a,e){var d=a.getPath().replace(/\.less$/,".css");eXide.util.message("Compiling less file to "+d);$.ajax({url:"modules/store.xql",type:"POST",data:{path:d,data:e,mime:"text/css"},dataType:"json",success:function(b){"error"==b.status?eXide.util.error(b.message):eXide.util.message(d+" stored.")},error:function(b){eXide.util.error(b.responseText)}})};Constr.prototype.createOutline=function(c,e){for(var d=new a(c.getSession(),0,0),b=d.stepForward();null!=b;){if("paren.lparen"==
b.type&&"{"==b.value){for(var b=[],f=d.getCurrentTokenRow(),h=new a(c.getSession(),f,d.getCurrentTokenColumn()),j;null!=(j=h.stepBackward())&&!(h.getCurrentTokenRow()<f);)b.push(j.value);b=b.reverse().join("");c.functions.push({type:eXide.edit.Document.TYPE_FUNCTION,name:b,source:c.getPath(),signature:b,row:d.getCurrentTokenRow(),column:d.getCurrentTokenColumn()});lastVar=""}b=d.stepForward()}e&&e(c)};return Constr}();eXide.namespace("eXide.edit.JavascriptModeHelper");
...
eXide.edit.Outline=function(){Constr=function(){this.currentDoc=null;this.templates=[];this.$loadTemplates()};Constr.prototype={getTemplates:function(a){for(var a=RegExp("^"+a),c=[],e=0;e<this.templates.length;e++)this.templates[e].name.match(a)&&c.push(this.templates[e]);return c},gotoDefinition:function(a,c){$.each(a.functions,function(a,d){c==d.name&&eXide.app.locate(d.type,""==d.source?null:d.source,c)})},updateOutline:function(a){var c=this;c.currentDoc=a;a.functions=[];$("#outline").fadeOut(100,
function(){$(this).empty();var e=a.getModeHelper();null!=e&&e.createOutline(a,function(){c.$outlineUpdate(a)})})},clearOutline:function(){$("#outline").empty()},$outlineUpdate:function(a){if(this.currentDoc==a){$.log("Updating outline");$("body").layout();eXide.app.resize();var c=$("#outline");c.empty();for(var e=0;e<a.functions.length;e++){var d=a.functions[e],b=document.createElement("li"),f=document.createElement("a");d.signature&&(f.title=d.signature);var h=$(f);d.type==eXide.edit.Document.TYPE_FUNCTION?
h.addClass("t_function"):h.addClass("t_variable");f.href=d.source?"#"+d.source:"#";"private"===d.visibility?h.addClass("private"):h.addClass("public");d.row&&(f.dataset.row=d.row);f.appendChild(document.createTextNode(d.name));b.appendChild(f);c.append(b);h.click(function(b){b.preventDefault();b=this.hash.substring(1);this.dataset.row?eXide.app.locate("function",b==""?null:b,parseInt(this.dataset.row)):$(this).hasClass("t_function")?eXide.app.locate("function",b==""?null:b,$(this).text()):eXide.app.locate("variable",
b==""?null:b,$(this).text())})}c.fadeIn(100)}},$loadTemplates:function(){var a=this;$.ajax({url:"templates/snippets.xml",dataType:"xml",type:"GET",success:function(c){$(c).find("snippet").each(function(){var c=$(this),d=c.attr("abbrev"),b=c.find("description").text(),c=c.find("code").text();a.templates.push({TYPE:eXide.edit.Document.TYPE_TEMPLATE,name:d,help:b,template:c})})}})}};return Constr}();eXide.namespace("eXide.edit.Document");
...
less:new eXide.edit.LessModeHelper(d),javascript:new eXide.edit.JavascriptModeHelper(d)}};eXide.util.oop.inherit(Constr,eXide.events.Sender);Constr.prototype.init=function(){0==this.documents.length&&this.newDocument()};Constr.prototype.exec=function(){if(this.activeDoc.getModeHelper()){var b=Array.prototype.slice.call(arguments,1);this.activeDoc.getModeHelper().exec(arguments[0],this.activeDoc,b)}else eXide.util.message("Not supported in this mode.")};Constr.prototype.getActiveDocument=function(){return this.activeDoc};
Constr.prototype.getText=function(){return this.activeDoc.getText()};Constr.prototype.newDocument=function(b,a){for(var c=0,d=0;d<this.documents.length;d++)this.documents[d].path.match(/^__new__(\d+)/)&&(c=parseInt(RegExp.$1));c++;d=b&&"string"==typeof b?new e(b):a&&"xquery"===a?new e('xquery version "3.0";\n'):new e("");c=new eXide.edit.Document("new-document "+c,"__new__"+c,d);a?c.setSyntax(a):c.setSyntax("text");this.$initDocument(c)};Constr.prototype.newDocumentWithText=function(b,a,c){b=new eXide.edit.Document(c.name,
...
function(b){var a=b.offset().left,b=a+b.outerWidth(),c=$("#tabs-container").innerWidth(),d=$("#tabs-container").scrollLeft();b>c?$("#tabs-container").scrollLeft(b-c):a<d&&(a<c?$("#tabs-container").scrollLeft(0):$("#tabs-container").scrollLeft(a));$.log("Scrolling to %d %d",a,$("#tabs-container").scrollLeft())};Constr.prototype.setTheme=function(a){$.log("Changing theme to %s",a);var c=this;c.loadTheme(a,function(){c.editor.setTheme("ace/theme/"+a)})};Constr.prototype.loadTheme=function(a,c){if(this.themes[a])return c();
...
a.lastChangeEvent=c;a.validateTimeout=setTimeout(function(){a.validate.apply(a)},2E3)}}};Constr.prototype.validate=function(){var a=this;a.$triggerEvent("validate",[a.activeDoc]);var c=a.activeDoc.getModeHelper();c&&c.validate&&(a.pendingCheck=!0,$.log("Running validation..."),c.validate(a.activeDoc,a.getText(),function(){a.pendingCheck=!1}))};Constr.prototype.evalError=function(a){var c=/.*line\s(\d+)/i.exec(a),d=-1;c&&(d=parseInt(c[1]));$.log("error in line %d",c[1]);this.editor.focus();this.editor.gotoLine(d);
