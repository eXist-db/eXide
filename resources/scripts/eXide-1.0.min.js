...
next:"start"},{token:"comment",regex:".*:\\)",next:"start"},{token:"comment",regex:".+"}]}};d.inherits(f,b);c.XQueryHighlightRules=f});
define("eXide/mode/behaviour/xquery",function(a,c){var d=a("ace/lib/oop"),e=a("ace/mode/behaviour").Behaviour,b=a("ace/mode/behaviour/cstyle").CstyleBehaviour,f=function(f){this.inherit(b,["braces","parens","string_dquotes"]);this.parent=f;this.add("brackets","insertion",function(b,f,e,a,c){return"\n"==c&&(f=e.getCursorPosition(),"</"==a.doc.getLine(f.row).substring(f.column,f.column+2))?(b=this.$getIndent(a.doc.getLine(f.row))+a.getTabString(),a=this.$getIndent(a.doc.getLine(f.row)),{text:"\n"+b+
"\n"+a,selection:[1,b.length,1,b.length]}):!1});this.add("comments","insertion",function(b,f,e,a,c){b=e.getCursorPosition();a=a.doc.getLine(b.row);if("\n"==c&&a.match(/^[\(\s]:/))return{text:"\n : ",selection:[1,3,1,3]};if(":"==c&&"("==a.substring(b.column-1,1))return $.log("cursor: %d",b.column),{text:":  :",selection:[2,2]}});this.add("slash","insertion",function(b,a,e,c,d){"/"==d&&(b=e.getCursorPosition(),a=c.doc.getLine(b.row),0<b.column&&"<"==a.charAt(b.column-1)&&(a=a.substring(0,b.column)+
"/"+a.substring(b.column),e=c.doc.getAllLines(),e[b.row]=a,f.exec("closeTag",e.join(c.doc.getNewLineCharacter()),b.row)));return!1})};d.inherits(f,e);c.XQueryBehaviour=f});
define("eXide/mode/xquery",function(a,c){var d=a("ace/lib/oop"),e=a("ace/mode/text").Mode,b=a("ace/tokenizer").Tokenizer,f=a("eXide/mode/xquery_highlight_rules").XQueryHighlightRules,h=a("eXide/mode/behaviour/xquery").XQueryBehaviour,j=a("ace/range").Range,g=function(a){this.$tokenizer=new b((new f).getRules());this.$behaviour=new h(a)};d.inherits(g,e);(function(){this.getNextLineIndent=function(b,f,a){b=this.$getIndent(f);f.match(/\s*(?:then|else|return|[{\(]|<\w+>)\s*$/)&&(b+=a);return b};this.checkOutdent=
function(b,f,a){return!/^\s+$/.test(f)?!1:/^\s*[\}\)]/.test(a)};this.autoOutdent=function(b,f,a){b=f.getLine(a).match(/^(\s*[\}\)])/);if(!b)return 0;var b=b[1].length,e=f.findMatchingBracket({row:a,column:b});if(!e||e.row==a)return 0;e=this.$getIndent(f.getLine(e.row));f.replace(new j(a,0,a,b-1),e)};this.$getIndent=function(b){return(b=b.match(/^(\s+)/))?b[1]:""};this.toggleCommentLines=function(b,f,a,e){for(var c=!0,h=/^\s*\(:(.*):\)/,b=a;b<=e;b++)if(!h.test(f.getLine(b))){c=!1;break}for(var d=new j(0,
0,0,0),b=a;b<=e;b++)a=f.getLine(b),d.start.row=b,d.end.row=b,d.end.column=a.length,f.replace(d,c?a.match(h)[1]:"(:"+a+":)")}}).call(g.prototype);c.Mode=g});
define("eXide/mode/behaviour/xml",function(a,c){var d=a("ace/lib/oop"),e=a("ace/mode/behaviour").Behaviour,b=a("ace/mode/behaviour/cstyle").CstyleBehaviour;a("eXide/mode/behaviour/xquery");var f=function(f){this.inherit(b,["braces","parens","string_dquotes"]);this.parent=f;this.add("brackets","insertion",function(b,f,a,e,c){return"\n"==c&&(f=a.getCursorPosition(),"</"==e.doc.getLine(f.row).substring(f.column,f.column+2))?(b=this.$getIndent(e.doc.getLine(f.row))+e.getTabString(),e=this.$getIndent(e.doc.getLine(f.row)),
{text:"\n"+b+"\n"+e,selection:[1,b.length,1,b.length]}):!1});this.add("slash","insertion",function(b,a,e,c,d){"/"==d&&(b=e.getCursorPosition(),a=c.doc.getLine(b.row),0<b.column&&"<"==a.charAt(b.column-1)&&(a=a.substring(0,b.column)+"/"+a.substring(b.column),e=c.doc.getAllLines(),e[b.row]=a,f.exec("closeTag",e.join(c.doc.getNewLineCharacter()),b.row)));return!1})};d.inherits(f,e);c.XMLBehaviour=f});
define("eXide/mode/xml",function(a,c){var d=a("ace/lib/oop"),e=a("ace/mode/xml").Mode,b=a("ace/tokenizer").Tokenizer,f=a("ace/mode/xml_highlight_rules").XmlHighlightRules,h=a("ace/mode/folding/xml").FoldMode,j=a("eXide/mode/behaviour/xml").XMLBehaviour,g=a("ace/range").Range,i=function(a){this.$tokenizer=new b((new f).getRules());this.$behaviour=new j(a);this.foldingRules=new h};d.inherits(i,e);(function(){this.toggleCommentLines=function(b,f,a,e){for(var c=!0,h=/^\s*<\!--(.*)--\>/,b=a;b<=e;b++)if(!h.test(f.getLine(b))){c=
!1;break}for(var d=new g(0,0,0,0),b=a;b<=e;b++)a=f.getLine(b),d.start.row=b,d.end.row=b,d.end.column=a.length,f.replace(d,c?a.match(h)[1]:"<\!--"+a+"--\>")}}).call(i.prototype);c.Mode=i});
define("eXide/mode/html",function(a,c){var d=a("ace/lib/oop"),e=a("ace/mode/html").Mode,b=a("ace/tokenizer").Tokenizer,f=a("ace/mode/html_highlight_rules").HtmlHighlightRules,h=a("ace/mode/folding/html").FoldMode,j=a("eXide/mode/behaviour/xml").XMLBehaviour,g=a("ace/range").Range,i=a("ace/mode/javascript").Mode,l=a("ace/mode/css").Mode,m=function(a){var e=new f;this.$tokenizer=new b(e.getRules());this.$behaviour=new j(a);this.foldingRules=new h;this.$embeds=e.getEmbeds();this.createModeDelegates({"js-":i,
"css-":l})};d.inherits(m,e);(function(){this.toggleCommentLines=function(b,f,a,e){for(var c=!0,h=/^\s*<\!--(.*)--\>/,b=a;b<=e;b++)if(!h.test(f.getLine(b))){c=!1;break}for(var d=new g(0,0,0,0),b=a;b<=e;b++)a=f.getLine(b),d.start.row=b,d.end.row=b,d.end.column=a.length,f.replace(d,c?a.match(h)[1]:"<\!--"+a+"--\>")}}).call(m.prototype);c.Mode=m});var eXide=eXide||{};eXide.namespace=function(a){var a=a.split("."),c=eXide,d;"eXide"==a[0]&&(a=a.slice(1));for(d=0;d<a.length;d++)"undefined"==typeof c[a[d]]&&(c[a[d]]={}),c=c[a[d]];return c};eXide.namespace("eXide.util");
eXide.util=function(){var a={dir1:"up",dir2:"left",firstpos1:15,firstpos2:15};return{popup:function(a,d,e,b){function f(b){j&&(j.empty(),b.find(".tooltip").each(function(){j.html($(this).html())}))}var h=$(a),j=d?$(d):null,g=null;e.sort(function(b,f){return b.label==f.label?0:b.label>f.label?1:-1});h.empty().css("display","block").focus();a=document.createElement("a");a.href="#";a.className="popup-close";a.appendChild(document.createTextNode("Close"));h.append(a);$(a).click(function(b){b.preventDefault();
h.css("display","none");j&&j.css("display","none")});a=document.createElement("ul");for(d=0;d<e.length;d++){var i=document.createElement("li");0==d&&(i.className="first");i.appendChild(document.createTextNode(e[d].label));if(e[d].tooltip){var l=document.createElement("span");l.className="tooltip";l.appendChild(document.createTextNode(e[d].tooltip));i.appendChild(l)}a.appendChild(i);$(i).click(function(){g.removeClass("selection");$(this).addClass("selection");g=$(this);f(g)});$(i).dblclick(function(){var f=
h.find("li").index(g);h.css("display","none");j&&j.css("display","none");b.call(null,e[f])})}h.append(a);g=h.find("ul li:first").addClass("selection");f(g);var m=$(a).css("position","absolute").scrollTop(0);j&&j.css({display:"block",top:m.offset().top+"px"});var n=m.innerHeight();$(h).keydown(function(a){a.preventDefault();if(40==a.which)a=g.next(),0<a.length&&(g.removeClass("selection"),a.addClass("selection"),g=a,a.position().top+a.height()>=n&&a.get(0).scrollIntoView(),f(a));else if(38==a.which)a=
g.prev(),0<a.length&&(g.removeClass("selection"),a.addClass("selection"),g=a,a.hasClass("first")?m.scrollTop(0):0>a.position().top&&a.get(0).scrollIntoView(),f(a));else if(13==a.which){var c=h.find("li").index(g);h.css("display","none");j&&j.css("display","none");$(h).unbind(a);b.call(null,e[c])}else return h.css("display","none"),j&&j.css("display","none"),$(h).unbind(a),b.call(null,null),!0;return!1})},supportsHtml5Storage:function(){try{return"localStorage"in window&&null!==window.localStorage}catch(a){return!1}},
normalizePath:function(a){if(!a)return a;for(var a=a.replace(/^xmldb:exist:\/\//,""),d=[],a=a.split("/"),e=a.length-1;-1<e;e--)".."==a[e]?e--:d.push(a[e]);return d.reverse().join("/")},parseSignature:function(a){var d=a.indexOf("(");if(-1<d){var e=a.substring(0,d+1),a=a.substring(d);if(a=a.match(/\$[\w:-_]+/g))for(d=0;d<a.length;d++)0<d&&(e+=", "),e+=a[d];return e+")"}return a},message:function(c){$.pnotify({pnotify_text:c,pnotify_shadow:!0,pnotify_hide:!0,pnotify_closer:!0,pnotify_opacity:0.75,pnotify_addclass:"stack-bottomright",
pnotify_stack:a})},error:function(c,d){var e={pnotify_text:c,pnotify_type:"error",pnotify_shadow:!0,pnotify_hide:!0,pnotify_addclass:"stack-bottomright",pnotify_stack:a};d&&(e.pnotify_title=d);$.pnotify(e)}}}();eXide.namespace("eXide.util.Dialog");
...
inputDialog=$("#eXide-dialog-input");inputDialog.dialog({modal:!0,autoOpen:!1,buttons:{OK:function(){$(this).dialog("close");null!=c&&c.apply($("eXide-dialog-input-body"),[])},Cancel:function(){$(this).dialog("close")}}})});return{message:function(c,e){null==e&&(e="");a.dialog("option","title",c);$("#eXide-dialog-message-body").html(e);$("#eXide-dialog-message-icon").attr("src","resources/images/information.png");a.dialog("open")},warning:function(c,e){null==e&&(e="");a.dialog("option","title",c);
$("#eXide-dialog-message-body").html(e);$("#eXide-dialog-message-icon").attr("src","resources/images/error.png");a.dialog("open")},input:function(a,e,b){c=b;inputDialog.dialog("option","title",a);$("#eXide-dialog-input-body").html(e);inputDialog.dialog("open")}}}();eXide.namespace("eXide.util.mimeTypes");
eXide.util.mimeTypes=function(){var a={xml:["text/xml","application/xml","application/xhtml+xml","application/atom+xml"],xquery:["application/xquery"],css:["text/css"],html:["text/html"],javascript:["application/x-javascript"],text:["text/text"],less:["application/less"]};return{getMime:function(a){var d=a.indexOf(";");-1<d&&(a=a.substring(0,d));return a},getLangFromMime:function(c){for(var d in a)for(var e=a[d],b=0;b<e.length;b++)if(c==e[b])return d;return"xquery"},getMimeFromLang:function(c){return(c=
a[c])?c[0]:"application/xquery"}}}();eXide.namespace("eXide.util.oop");eXide.util.oop.inherit=function(){var a=function(){};return function(c,d){a.prototype=d.prototype;c.prototype=new a;c.super_=d.prototype;c.prototype.constructor=c}}();eXide.util.oop.extend=function(){return function(a,c){for(var d in c)c.hasOwnProperty(d)&&(a[d]=c[d])}}();(function(a){a.log=function(){window.console&&window.console.log&&console.log.apply(window.console,arguments)};a.fn.log=function(){a.log(this);return this}})(jQuery);eXide.namespace("eXide.events.Sender");eXide.events.Sender=function(){Constr=function(){};Constr.prototype={addEventListener:function(a,c,d){this.events=this.events||{};var e=this.events[a];e||(e=[],this.events[a]=e);e.push({obj:c,callback:d})},$triggerEvent:function(a,c){this.events=this.events||{};var d=this.events[a];if(d)for(var e=0;e<d.length;e++)d[e].callback.apply(d[e].obj,c)}};return Constr}();eXide.namespace("eXide.edit.commands");
eXide.edit.commands=function(){function a(a){return{win:a[0],mac:a[1],sender:"editor"}}var c=require("ace/lib/useragent"),d={};return{init:function(e){commands=e.editor.commands;$.ajax({url:"keybindings.js",dataType:"json",async:!1,success:function(b){commands.addCommand({name:"fold",bindKey:a(b.fold),exec:function(b){b.editor.session.toggleFold(!1)},readOnly:!0});commands.addCommand({name:"unfold",bindKey:a(b.unfold),exec:function(b){b.editor.session.toggleFold(!0)},readOnly:!0});commands.addCommand({name:"saveDocument",
...
commands.addCommand({name:"autocomplete",bindKey:a(b.autocomplete),exec:function(){e.autocomplete()}});commands.addCommand({name:"nextTab",bindKey:a(b.nextTab),exec:function(){e.nextTab()}});commands.addCommand({name:"previousTab",bindKey:a(b.previousTab),exec:function(){e.previousTab()}});commands.addCommand({name:"functionDoc",bindKey:a(b.functionDoc),exec:function(){e.exec("showFunctionDoc")}});commands.addCommand({name:"gotoDefinition",bindKey:a(b.gotoDefinition),exec:function(){e.exec("gotoDefinition")}});
commands.addCommand({name:"searchIncremental",bindKey:a(b.searchIncremental),exec:function(){e.quicksearch.start()}});commands.addCommand({name:"findModule",bindKey:a(b.findModule),exec:function(){var b=e.getActiveDocument();eXide.find.Modules.select(b.syntax)}});commands.addCommand({name:"indentOrParam",bindKey:a(b.indentOrParam),exec:function(){var b=e.getActiveDocument();(!b.template||!b.template.nextParam())&&e.editor.indent()}});commands.addCommand({name:"escape",bindKey:a(b.escape),exec:function(){e.getActiveDocument().template=
null;e.editor.clearSelection()}});commands.addCommand({name:"dbManager",bindKey:a(b.dbManager),exec:function(){eXide.app.manage()}});commands.addCommand({name:"toggleComment",bindKey:a(b.toggleComment),exec:function(){e.editor.toggleCommentLines()}});commands.addCommand({name:"synchronize",bindKey:a(b.synchronize),exec:function(){eXide.app.synchronize()}});commands.addCommand({name:"preferences",bindKey:a(b.preferences),exec:function(){eXide.app.showPreferences()}});commands.addCommand({name:"openApp",
bindKey:a(b.openApp),exec:function(){eXide.app.openApp()}});b=e.editor.commands;for(key in b.commands){var f=b.commands[key],h;f.bindKey&&(h=c.isMac?f.bindKey.mac:f.bindKey.win);d[f.name]=h}}})},help:function(a,b){$(a).find("table").each(function(){this.innerHTML="";var a=b.editor.commands;for(key in a.commands){var e=a.commands[key],d=document.createElement("tr"),g=document.createElement("td");g.appendChild(document.createTextNode(e.name));d.appendChild(g);g=document.createElement("td");e.bindKey&&
(c.isMac?g.appendChild(document.createTextNode(e.bindKey.mac)):g.appendChild(document.createTextNode(e.bindKey.win)));d.appendChild(g);this.appendChild(d)}})},getShortcut:function(a){return d[a]}}}();eXide.namespace("eXide.edit.ModeHelper");
eXide.edit.ModeHelper=function(){Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.commands={};this.addCommand("locate",this.locate)};Constr.prototype={addCommand:function(a,c){this.commands||(this.commands={});this.commands[a]=c},exec:function(a,c,d){if(this.commands&&this.commands[a]){for(var c=[c],e=0;e<d.length;e++)c.push(d[e]);$.log("Calling command %s ...",a);this.commands[a].apply(this,c)}else eXide.util.message("Not supported in this mode.")},createOutline:function(){},
documentSaved:function(){},locate:function(a,c,d){"number"==typeof d&&(this.editor.gotoLine(d+1),this.editor.focus());return!1}};return Constr}();eXide.namespace("eXide.edit.XQueryModeHelper");
eXide.edit.XQueryModeHelper=function(){function a(a){var b;b=a.line?a["#text"]:a;var f=d.exec(b),c=-1;f?c=parseInt(f[1])-1:a.line&&(c=parseInt(a.line)-1);return{line:c,msg:b}}var c=/^[\$\w:\-_\.]+/;Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.funcDefRe=/declare\s+((?:%[\w\:\-]+(?:\([^\)]*\))?\s*)*)function\s+([^\(]+)\(/g;this.varDefRe=/declare\s+(?:%\w+\s+)*variable\s+\$[^\s;]+/gm;this.varRe=/declare\s+(?:%\w+\s+)*variable\s+(\$[^\s;]+)/;this.parseImportRe=/import\s+module\s+namespace\s+[^=]+\s*=\s*["'][^"']+["']\s*at\s+["'][^"']+["']\s*;/g;
this.moduleRe=/import\s+module\s+namespace\s+([^=\s]+)\s*=\s*["']([^"']+)["']\s*at\s+["']([^"']+)["']\s*;/;this.addCommand("showFunctionDoc",this.showFunctionDoc);this.addCommand("gotoDefinition",this.gotoDefinition);this.addCommand("locate",this.locate);this.addCommand("closeTag",this.closeTag);this.addCommand("importModule",this.importModule)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.closeTag=function(e,b,f){var e="xmldb:exist://"+e.getBasePath(),c=this;$.ajax({type:"POST",
url:"modules/compile.xql",data:{q:b,base:e},dataType:"json",success:function(b){"fail"==b.result&&(b=a(b.error),b.line<=f&&(b=/constructor:\s(.*)$/.exec(b.msg),0<b.length&&c.editor.insert(b[1]+">")))},error:function(){}})};Constr.prototype.validate=function(a,b,f){$.log("Running validation...");var c=this,d="xmldb:exist://"+a.getBasePath();$.ajax({type:"POST",url:"modules/compile.xql",data:{q:b,base:d},dataType:"json",success:function(b){c.compileError(b,a);f.call(this,!0)},error:function(b,a){f.call(this,
!0);$.log("Compile error: %s - %s",a,b.responseText)}})};Constr.prototype.compileError=function(e,b){if("fail"==e.result){var f=a(e.error),c=[{row:f.line,text:f.msg,type:"error"}];this.parent.updateStatus(f.msg,b.getPath()+"#"+f.line);b.getSession().setAnnotations(c)}else this.parent.clearErrors(),this.parent.updateStatus("")};Constr.prototype.autocomplete=function(a){require("pilot/lang");for(var b=require("ace/range").Range,f=this.editor.getSelection(),c=a.getSession(),f=f.getSelectionLead(),c=
c.getDisplayLine(f.row),d=f.column-1,g=f.column;0<=d;){var i=c.substring(d,g);if(i.match(/^\$[\w:\-_\.]+$/))break;if(!i.match(/^[\w:\-_\.]+$/)){d++;break}d--}c=c.substring(d,g);$.log("completing token: %s",c);b=new b(f.row,d,f.row,g);f=this.editor.renderer.textToScreenCoordinates(f.row,f.column);d=this.parent.getHeight();f.pageY+150>d&&(f.pageY=d-150,$.log("window height: %i, pageY: %i",d,f.pageY));$("#autocomplete-box").css({left:f.pageX+"px",top:f.pageY+10+"px"});$("#autocomplete-help").css({left:f.pageX+
324+"px",top:f.pageY+10+"px"});0==c.length?this.templateLookup(a,c,b,!0):this.functionLookup(a,c,b,!0);return!0};Constr.prototype.$localVars=function(a,b){for(var f=[],c=/declare function|};/,d=/let \$[\w\:]+|for \$[\w\:]+|\$[\w\:]+\)/,g=/\$[\w\:]+/,i=RegExp("^\\"+a),l=this.editor.getSession(),m=b.start.row;-1<m;){var n=l.getDisplayLine(m),o;if(o=n.match(d))$.log("Var: %s",o[0]),o=o[0].match(g),o[0].match(i)&&f.push({name:o[0],type:"variable"});if(n.match(c)){$.log("Stop: %s",n);break}m--}return f};
Constr.prototype.functionLookup=function(a,b,f,c){var d=this;$.ajax({url:"modules/docs.xql",dataType:"text",type:"POST",data:{prefix:b},success:function(g){var g=$.parseJSON(g),i=[],l;"$"==b.substring(0,1)?(l="^\\"+b,i=d.$localVars(b,f,c)):l="^"+b;var m=RegExp(l);$.each(a.functions,function(b,a){a.name.match(m)&&i.push(a)});g&&(i=i.concat(g));g=[];for(l=0;l<i.length;l++){var n={label:i[l].signature?i[l].signature:i[l].name,type:i[l].type};i[l].help&&(n.tooltip=i[l].help);g.push(n)}d.$addTemplates(b,
g);d.$showPopup(a,f,g)},error:function(b,a){eXide.util.error(a)}})};Constr.prototype.templateLookup=function(a,b,f){var c=[];this.$addTemplates(b,c);this.$showPopup(a,f,c)};Constr.prototype.$addTemplates=function(a,b){for(var f=this.parent.outline.getTemplates(a),c=0;c<f.length;c++)b.push({type:"template",label:"[S] "+f[c].name,tooltip:f[c].help,template:f[c].template})};Constr.prototype.$showPopup=function(a,b,f){var c=this;eXide.util.popup($("#autocomplete-box"),$("#autocomplete-help"),f,function(f){if(f){var d=
f.label;"template"==f.type?d=f.template:"function"==f.type&&(d=eXide.util.parseSignature(d));a.template=new eXide.edit.Template(c.parent,b,d,f.type);a.template.insert()}c.editor.focus()})};Constr.prototype.getFunctionAtCursor=function(a){var b=a.row,b=this.editor.getSession().getDisplayLine(b),f=a.column;do f--;while(0<=f&&b.charAt(f).match(c));f++;for(a=a.column;a<b.length&&b.charAt(a).match(c);)a++;return b.substring(f,a)};Constr.prototype.showFunctionDoc=function(a){var b=this.editor.getSelection().getSelectionLead(),
...
f);break;default:this.gotoVarDecl(a,f)}};Constr.prototype.gotoFunction=function(a,b){$.log("Goto function %s",b);var f=this.getModuleNamespacePrefix();null!=f&&(b=b.replace(/[^:]+:/,f+":"));for(var f=RegExp("function\\s+"+b+"\\s*\\("),c=a.$session.getLength(),d=0;d<c;d++)if(a.$session.getLine(d).match(f)){this.editor.gotoLine(d+1);this.editor.focus();break}};Constr.prototype.gotoVarDecl=function(a,b){var f=this.getModuleNamespacePrefix();null!=f&&(b=b.replace(/[^:]+:/,"$"+f+":"));$.log("Goto variable declaration %s",
b);for(var f=RegExp("variable\\s+\\"+b),c=a.$session.getLength(),d=0;d<c;d++)if(a.$session.getLine(d).match(f)){this.editor.gotoLine(d+1);this.editor.focus();break}};Constr.prototype.getModuleNamespacePrefix=function(){for(var a=/^\s*module\s+namespace\s+([^=\s]+)\s*=/,b=this.parent.getActiveDocument().$session.getLength(),f=0;f<b;f++){var c=this.parent.getActiveDocument().$session.getLine(f).match(a);if(c)return c[1]}return null};Constr.prototype.importModule=function(a,b,f,c){$.log("location = %s path = %s",
c,a.path);a=a.getBasePath();c=0===c.lastIndexOf(a,0)?c.substring(a.length+1):"xmldb:exist://"+c;this.editor.insert("import module namespace "+b+'="'+f+'" at "'+c+'";\n')};Constr.prototype.createOutline=function(a,b){var f=a.getText();this.$parseLocalFunctions(f,a);b&&b(a);(f=this.$parseImports(f))&&this.$resolveImports(a,f,b)};Constr.prototype.$sortFunctions=function(a){a.functions.sort(function(b,a){return b.source&&!a.source?1:a.source&&!b.source?-1:b.name==a.name?0:b.name>a.name?1:-1})};Constr.prototype.$parseLocalFunctions=
function(a,b){for(b.functions=[];;){var f=this.funcDefRe.exec(a);if(null==f)break;var c=this.funcDefRe.lastIndex,d=this.$findMatchingParen(a,c),g=3==f.length?f[2]:f[1],f=3==f.length?f[1]:"public";-1!==f.indexOf("%private")&&(f="private");b.functions.push({type:eXide.edit.Document.TYPE_FUNCTION,name:g,visibility:f,signature:g+"("+a.substring(c,d)+")"})}if(c=a.match(this.varDefRe))for(d=0;d<c.length;d++)g=this.varRe.exec(c[d]),b.functions.push({type:eXide.edit.Document.TYPE_VARIABLE,name:g[1]});this.$sortFunctions(b)};
Constr.prototype.$findMatchingParen=function(a,b){for(var f=1,c=b;c<a.length;c++){var d=a.charAt(c);if(")"==d){if(f-=1,0==f)return c}else"("==d&&(f+=1)}return-1};Constr.prototype.$parseImports=function(a){return a.match(this.parseImportRe)};Constr.prototype.$resolveImports=function(a,b,f){for(var c=this,d=[],g=[],i=0;i<b.length;i++){var l=this.moduleRe.exec(b[i]);null!=l&&4==l.length&&(g.push("prefix="+encodeURIComponent(l[1])),g.push("uri="+encodeURIComponent(l[2])),g.push("source="+encodeURIComponent(l[3])))}b=
"xmldb:exist://"+a.getBasePath();g.push("base="+encodeURIComponent(b));$.ajax({url:"outline",dataType:"json",type:"POST",data:g.join("&"),success:function(b){if(null!=b){for(var b=b.modules,i=0;i<b.length;i++){var g=b[i].functions;if(g)for(var l=0;l<g.length;l++)d.push({type:eXide.edit.Document.TYPE_FUNCTION,name:g[l].name,signature:g[l].signature,visibility:g[l].visibility,source:b[i].source});if(g=b[i].variables)for(l=0;l<g.length;l++)d.push({type:eXide.edit.Document.TYPE_VARIABLE,name:"$"+g[l],
source:b[i].source})}a.functions=a.functions.concat(d);c.$sortFunctions(a);f&&f(a)}}});return d};var d=/.*line:?\s(\d+)/i;return Constr}();eXide.namespace("eXide.edit.XMLModeHelper");
eXide.edit.XMLModeHelper=function(){Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.addCommand("closeTag",this.closeTag)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.closeTag=function(a,c,d){a.getBasePath();var e=this;$.ajax({type:"POST",url:"modules/validate-xml.xql",data:{xml:c},dataType:"json",success:function(b){b.status&&"invalid"==b.status&&parseInt(b.message.line)-1<=d&&(b=/element type \"([^\"]+)\"/.exec(b.message["#text"]),0<b.length&&e.editor.insert(b[1]+
">"))},error:function(){}})};Constr.prototype.validate=function(a,c,d){var e=this;$.ajax({type:"POST",url:"modules/validate-xml.xql",data:{xml:c},dataType:"json",success:function(b){e.compileError(b,a);d.call(this,!0)},error:function(b,a){d.call(this,!0);$.log("Compile error: %s - %s",a,b.responseText)}})};Constr.prototype.compileError=function(a,c){$.log("Validation returned %o",a);if(a.status&&"invalid"==a.status){var d=[{row:parseInt(a.message.line)-1,text:a.message["#text"],type:"error"}];$.log("annotation: %o",
d);this.parent.updateStatus(a.message["#text"],c.getPath()+"#"+a.message.line);c.getSession().setAnnotations(d)}else this.parent.clearErrors(),this.parent.updateStatus("")};return Constr}();eXide.namespace("eXide.edit.LessModeHelper");
eXide.edit.LessModeHelper=function(){var a=require("ace/token_iterator").TokenIterator;Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.addCommand("locate",this.locate)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.documentSaved=function(a){var d=this,e=a.getText(),b="/exist/apps/"+a.getBasePath().replace(/^\/db\//,"")+"/";(new less.Parser({paths:[b],optimization:3})).parse(e,function(b,e){if(b)return $.error(b);d.saveCSS(a,e.toCSS())})};Constr.prototype.saveCSS=
function(a,d){var e=a.getPath().replace(/\.less$/,".css");eXide.util.message("Compiling less file to "+e);$.ajax({url:"modules/store.xql",type:"POST",data:{path:e,data:d,mime:"text/css"},dataType:"json",success:function(b){"error"==b.status?eXide.util.error(b.message):eXide.util.message(e+" stored.")},error:function(b){eXide.util.error(b.responseText)}})};Constr.prototype.createOutline=function(c,d){for(var e=new a(c.getSession(),0,0),b=e.stepForward(),f="";null!=b;)"variable"==b.type||"keyword"==
b.type?(0<f.length&&(f+=" "),f+=b.value):"paren.lparen"==b.type&&(c.functions.push({type:eXide.edit.Document.TYPE_FUNCTION,name:f,source:c.getPath(),signature:f,row:e.getCurrentTokenRow(),column:e.getCurrentTokenColumn()}),f=""),b=e.stepForward();d&&d(c)};return Constr}();eXide.namespace("eXide.edit.JavascriptModeHelper");
eXide.edit.JavascriptModeHelper=function(){var a=require("ace/token_iterator").TokenIterator;Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.addCommand("locate",this.locate)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.createOutline=function(c,d){for(var e=new a(c.getSession(),0,0),b=e.stepForward();null!=b;)"entity.name.function"==b.type&&c.functions.push({type:eXide.edit.Document.TYPE_FUNCTION,name:b.value,signature:b.value,row:e.getCurrentTokenRow(),
column:e.getCurrentTokenColumn()}),b=e.stepForward();d&&d(c)};return Constr}();eXide.namespace("eXide.edit.CssModeHelper");
eXide.edit.CssModeHelper=function(){var a=require("ace/token_iterator").TokenIterator;Constr=function(a){this.parent=a;this.editor=this.parent.editor;this.addCommand("locate",this.locate)};eXide.util.oop.inherit(Constr,eXide.edit.ModeHelper);Constr.prototype.createOutline=function(c,d){for(var e=new a(c.getSession(),0,0),b=e.stepForward(),f="";null!=b;)"variable"==b.type||"keyword"==b.type?(0<f.length&&(f+=" "),f+=b.value):"paren.lparen"==b.type&&(c.functions.push({type:eXide.edit.Document.TYPE_FUNCTION,name:f,
source:c.getPath(),signature:f,row:e.getCurrentTokenRow(),column:e.getCurrentTokenColumn()}),f=""),b=e.stepForward();d&&d(c)};return Constr}();eXide.namespace("eXide.edit.Outline");
eXide.edit.Outline=function(){Constr=function(){this.currentDoc=null;this.templates=[];this.$loadTemplates()};Constr.prototype={getTemplates:function(a){for(var a=RegExp("^"+a),c=[],d=0;d<this.templates.length;d++)this.templates[d].name.match(a)&&c.push(this.templates[d]);return c},gotoDefinition:function(a,c){$.each(a.functions,function(a,e){c==e.name&&eXide.app.locate(e.type,""==e.source?null:e.source,c)})},updateOutline:function(a){var c=this;c.currentDoc=a;a.functions=[];$("#outline").fadeOut(100,
function(){$(this).empty();var d=a.getModeHelper();null!=d&&d.createOutline(a,function(){c.$outlineUpdate(a)})})},clearOutline:function(){$("#outline").empty()},$outlineUpdate:function(a){if(this.currentDoc==a){$.log("Updating outline");$("body").layout();eXide.app.resize();var c=$("#outline");c.empty();for(var d=0;d<a.functions.length;d++){var e=a.functions[d],b=document.createElement("li"),f=document.createElement("a");e.signature&&(f.title=e.signature);var h=$(f);e.type==eXide.edit.Document.TYPE_FUNCTION?
h.addClass("t_function"):h.addClass("t_variable");f.href=e.source?"#"+e.source:"#";"private"===e.visibility?h.addClass("private"):h.addClass("public");e.row&&(f.dataset.row=e.row);f.appendChild(document.createTextNode(e.name));b.appendChild(f);c.append(b);h.click(function(b){b.preventDefault();b=this.hash.substring(1);this.dataset.row?eXide.app.locate("function",b==""?null:b,parseInt(this.dataset.row)):$(this).hasClass("t_function")?eXide.app.locate("function",b==""?null:b,$(this).text()):eXide.app.locate("variable",
b==""?null:b,$(this).text())})}c.fadeIn(100)}},$loadTemplates:function(){var a=this;$.ajax({url:"templates/snippets.xml",dataType:"xml",type:"GET",success:function(c){$(c).find("snippet").each(function(){var c=$(this),e=c.attr("abbrev"),b=c.find("description").text(),c=c.find("code").text();a.templates.push({TYPE:eXide.edit.Document.TYPE_TEMPLATE,name:e,help:b,template:c})})}})}};return Constr}();eXide.namespace("eXide.edit.Document");
eXide.edit.Document=function(){Constr=function(a,c,d){this.name=a;this.path=c;this.mime=null;this.syntax="xquery";this.saved=!1;this.editable=!0;this.functions=[];this.helper=null;this.history=[];this.$session=d;a=eXide.app.getPreference("softWrap");this.$session.setUseWrapMode(0!=a);0<a?this.$session.setWrapLimitRange(a,a):0>a&&this.$session.setWrapLimitRange(null,null);this.$session.setFoldStyle("markbegin")};Constr.TYPE_FUNCTION="function";Constr.TYPE_VARIABLE="variable";Constr.TYPE_TEMPLATE="template";
...
eXide.edit.Editor=function(){var a=require("ace/virtual_renderer").VirtualRenderer,c=require("ace/editor").Editor,d=require("ace/edit_session").EditSession,e=require("ace/undomanager").UndoManager;Constr=function(b,f,d){var e=this;e.container=b;e.menubar=f;e.projects=d;e.documents=[];e.activeDoc=null;e.tabCounter=0;e.newDocCounter=0;e.pendingCheck=!1;e.themes={};b=new a(e.container,"ace/theme/eclipse");b.setShowGutter(!0);this.editor=new c(b);this.editor.setBehavioursEnabled(!0);this.editor.setShowFoldWidgets(!0);
this.editor.setFadeFoldWidgets(!1);eXide.edit.commands.init(e);this.outline=new eXide.edit.Outline;this.addEventListener("activate",this.outline,this.outline.updateOutline);this.addEventListener("validate",this.outline,this.outline.updateOutline);this.addEventListener("close",this.outline,this.outline.clearOutline);this.status=document.getElementById("error-status");$(this.status).click(function(b){b.preventDefault();var a=this.pathname,b=this.hash.substring(1);if(a=e.getDocument(a))e.switchTo(a),
e.editor.gotoLine(parseInt(b)+1)});this.quicksearch=new eXide.find.IncrementalSearch($("#search-box"),this.editor);var g=$("#tabs-container");g.css({overflow:"hidden"});g.mousemove(function(b){var a=g.find("ul"),f=g.width(),a=a.find("li:last-child"),a=a[0].offsetLeft+a.outerWidth(),b=(b.pageX-g.offset().left)*(a-f)/f;g.scrollLeft(b)});this.lastChangeEvent=(new Date).getTime();this.validateTimeout=null;e.modes={xquery:new eXide.edit.XQueryModeHelper(e),xml:new eXide.edit.XMLModeHelper(e),html:new eXide.edit.XMLModeHelper(e),
javascript:new eXide.edit.JavascriptModeHelper(e)}};eXide.util.oop.inherit(Constr,eXide.events.Sender);Constr.prototype.init=function(){0==this.documents.length&&this.newDocument()};Constr.prototype.exec=function(){if(this.activeDoc.getModeHelper()){var b=Array.prototype.slice.call(arguments,1);this.activeDoc.getModeHelper().exec(arguments[0],this.activeDoc,b)}else eXide.util.message("Not supported in this mode.")};Constr.prototype.getActiveDocument=function(){return this.activeDoc};Constr.prototype.getText=
function(){return this.activeDoc.getText()};Constr.prototype.newDocument=function(b,a){for(var c=0,e=0;e<this.documents.length;e++)this.documents[e].path.match(/^__new__(\d+)/)&&(c=parseInt(RegExp.$1));c++;e=b&&"string"==typeof b?new d(b):a&&"xquery"===a?new d('xquery version "3.0";\n'):new d("");c=new eXide.edit.Document("new-document "+c,"__new__"+c,e);a?c.setSyntax(a):c.setSyntax("text");this.$initDocument(c)};Constr.prototype.newDocumentWithText=function(b,a,c){b=new eXide.edit.Document(c.name,
c.path,new d(b));b.editable=c.writable;b.mime=a;b.syntax=eXide.util.mimeTypes.getLangFromMime(a);b.saved=!1;c.line&&(b.addToHistory(c.line),this.editor.gotoLine(c.line));this.$initDocument(b)};Constr.prototype.openDocument=function(b,a,c){c.writable?eXide.util.message("Opening "+c.path):eXide.util.message("Opening "+c.path+" readonly!");b=new eXide.edit.Document(c.name,c.path,new d(b));b.editable=c.writable;b.mime=a;b.syntax=eXide.util.mimeTypes.getLangFromMime(a);b.saved=!0;c.line&&(b.addToHistory(c.line),
a=b.$session.getSelection(),a.clearSelection(),a.moveCursorTo(c.line,1),b.$session.setScrollTop(c.line));$.log("opening %s, mime: %s, syntax: %s, line: %i",c.name,b.mime,b.syntax,c.line);this.$initDocument(b)};Constr.prototype.$initDocument=function(b){var a=this;a.$setMode(b,!1);b.$session.setUndoManager(new e);b.$session.addEventListener("change",function(){b.saved&&(b.saved=!1,a.updateTabStatus(b.path,b));a.triggerCheck()});a.addTab(b);a.editor.setSession(b.$session);a.editor.resize();a.editor.focus()};
...
b;a.documents.push(b);a.$triggerEvent("activate",[b]);a.scrollToTab($(i))};Constr.prototype.nextTab=function(){if(!(2>this.documents.length)){for(var a=0,c=0;c<this.documents.length;c++)if(this.documents[c]==this.activeDoc){a=c;break}a==this.documents.length-1?a=0:a++;this.switchTo(this.documents[a])}};Constr.prototype.previousTab=function(){if(!(2>this.documents.length)){for(var a=0,c=0;c<this.documents.length;c++)if(this.documents[c]==this.activeDoc){a=c;break}0==a?a=this.documents.length-1:a--;
this.switchTo(this.documents[a])}};Constr.prototype.switchTo=function(a){this.editor.setSession(a.$session);this.editor.resize();this.activeDoc=a;var c=this;$("#tabs a").each(function(){var d=$(this);this.title==a.path?(d.addClass("active"),c.scrollToTab(d)):d.removeClass("active")});this.updateStatus("");this.$triggerEvent("activate",[a])};Constr.prototype.updateTabStatus=function(a,c){var d;d=c.saved?c.name:c.name+"*";$('#tabs a[title="'+a+'"]').attr("title",c.path).text(d)};Constr.prototype.scrollToTab=
...
d.getCurrentLine())}else localStorage["eXide."+a+".path"]=d.path,localStorage["eXide."+a+".name"]=d.name,localStorage["eXide."+a+".mime"]=d.mime,localStorage["eXide."+a+".writable"]=d.editable?"true":"false",localStorage["eXide."+a+".last-line"]=d.getCurrentLine(),d.saved||(localStorage["eXide."+a+".data"]=d.getText());a++});localStorage["eXide.documents"]=a};return Constr}();$(document).ready(function(){var a;var c=window.location.search.substr(1).split("&");if(""==c)a={};else{for(var d={},e=0;e<c.length;++e){var b=c[e].split("=");2==b.length&&(d[b[0]]=decodeURIComponent(b[1].replace(/\+/g," ")))}a=d}eXide.app.init(function(b){var c=a.open,d=a.snip;c&&!b[c]?eXide.app.findDocument(a.open):d&&eXide.app.newDocument(d);"function"==typeof eXide_onload&&eXide_onload(eXide.app)})});eXide.namespace("eXide.app");
eXide.app=function(){var a,c,d,e,b,f=0,h=0,j=0,g=0;return{init:function(f){var h=new eXide.util.Menubar($(".menu"));e=new eXide.edit.Projects;a=new eXide.edit.Editor(document.getElementById("editor"),h);c=new eXide.edit.PackageEditor(e);d=new eXide.browse.Browser(document.getElementById("open-dialog"));c.addEventListener("change",null,function(){d.onChange();eXide.app.openDocument()});b=new eXide.util.Preferences(a);eXide.app.initGUI(h);h=eXide.app.restoreState();a.init();a.addEventListener("outlineChange",
...
eXide.edit.Projects=function(){Constr=function(){this.projects={}};Constr.prototype.getProject=function(a,c){var d=this;$.getJSON("modules/deployment.xql",{info:a},function(a){if(a){var b=d.projects[a.abbrev];b?eXide.util.oop.extend(b,a):(b=a,d.projects[a.abbrev]=b);"function"==typeof c&&c(b)}else eXide.util.error("Application not found: The document currently opened in the editor should belong to an application package.")})};Constr.prototype.getProjectFor=function(a){for(k in this.projects){var c=this.projects[k];
...
Constr.prototype.runApp=function(a){this.projects.getProject(a,function(a){var d="/exist/apps/"+a.root.replace(/^\/db\//,"")+"/";eXide.util.Dialog.message("Run Application "+a.abbrev,'<p>Click on the following link to open your application:</p><center><a href="'+d+'" target="_new">'+d+"</a></center>")})};Constr.prototype.saveState=function(){this.projects.saveState()};Constr.prototype.restoreState=function(){this.projects.restoreState()};return Constr}();eXide.namespace("eXide.edit.Template");
eXide.edit.Template=function(){var a=/\n/;Constr=function(c,d,e,b){var f=e.split(a).length;this.code=e;this.editor=c.editor;this.range=d;this.type=b;this.startLine=d.start.row;this.endLine=this.startLine+f-1;$.log("startLine = %i, endLine = %i",this.startLine,this.endLine);this.currentLine=this.startLine;this.lineOffset=this.startColumn=d.start.column;this.regex=/\$[\w\-:_]+/g;0<this.startColumn&&(this.regex.lastIndex=this.startColumn)};Constr.prototype={insert:function(){this.editor.getSession().remove(this.range);this.editor.insert(this.code);
var a=this.editor.getSelection().getSelectionLead();"$"!=this.code.substring(0,1)&&0<a.column&&this.editor.navigateLeft();"variable"!=this.type&&this.nextParam();this.editor.focus()},nextParam:function(){var a=this.editor.getSession(),d=this.editor.getSelection(),e=d.getSelectionLead();$.log("lead.row = %i startLine = %i",e.row,this.startLine);if(e.row<this.startLine||e.row>this.endLine)return!1;for(var b=e=!1;this.currentLine<=this.endLine;){var f=a.getDisplayLine(this.currentLine);$.log("Checking line %s",
f);if(f=this.regex.exec(f)){$.log("Matched %s",f[0]);d.setSelectionAnchor(this.currentLine,f.index);d.selectTo(this.currentLine,f.index+f[0].length);this.lineOffset=f.index;b=!0;break}else this.lineOffset=0,this.currentLine++;this.currentLine>this.endLine&&!e&&($.log("loop %i",this.startColumn),this.currentLine=this.startLine,0<this.startColumn&&(this.lineOffset=this.startColumn,this.regex.lastIndex=this.lineOffset),e=!0)}return b}};return Constr}();eXide.namespace("eXide.util.Help");
...
eXide.browse.CollectionBrowser=function(){Constr=function(a){var c=this;this.events={activate:[]};this.selected=null;var d=document.createElement("div");d.className="eXide-browse-collections eXide-browse-content";a.append(d);this.container=$(d);this.container.dynatree({persist:!1,rootVisible:!1,initAjax:{url:"modules/collections.xql"},clickFolderMode:1,autoFocus:!0,keyboard:!1,onActivate:function(a){var b=a.data.key;$.log("activate %s: is writable: %s",b,a.data.writable);c.selected=b;c.$triggerEvent("activate",
[b,a.data.writable])},onPostInit:function(a){$.log("Reloading: %s",a);c.select(c.selected)}})};eXide.util.oop.inherit(Constr,eXide.events.Sender);Constr.prototype.getSelection=function(){return this.selected};Constr.prototype.getSelected=function(){return{isCollection:!1,name:this.selected}};Constr.prototype.select=function(a){var c=this.container.dynatree("getTree"),d=null;a&&(d=c.getNodeByKey(a),null==d&&(a=a.substring(0,a.lastIndexOf("/")),$.log("Activating parent collection %s",a),d=c.getNodeByKey(a)));
null==d&&(a="/db",d=c.getNodeByKey(a));null!=d&&(this.selected=a);d.activate();d.expand(!0)};Constr.prototype.reload=function(){$.log("Reloading tree...");this.container.dynatree("getTree").reload()};Constr.prototype.createCollection=function(){var a=this;eXide.app.$checkLogin()&&eXide.util.Dialog.input("Create Collection",'<label for="collection">Name: </label><input type="text" name="collection" id="eXide-browse-collection-name"/>',function(){$.getJSON("modules/collections.xql",{create:$("#eXide-browse-collection-name").val(),
...
a.message)})});this.grid.onViewportChanged.subscribe(function(){var a=b.grid.getViewport();b.load(a.top,a.bottom)});$("#resource-properties-dialog").dialog({title:"Resource/collection properties",modal:!0,autoOpen:!1,height:320,width:460,buttons:{Cancel:function(){$(this).dialog("close")},Apply:function(){var a=this,c=b.grid.getSelectionModel().getSelectedRows();if(0!=c.length){for(var d=[],f=0;f<c.length;f++)d.push(b.collection+"/"+b.data[c[f]].name);c=$("form",a).serialize();c=c+"&"+$.param({"modify[]":d});
$.getJSON("modules/collections.xql",c,function(){$(a).dialog("close");b.reload()})}}}})};eXide.util.oop.inherit(Constr,eXide.events.Sender);Constr.prototype.setMode=function(a){this.mode=a;"manage"==a?this.grid.setOptions(d):this.grid.setOptions(c)};Constr.prototype.resize=function(){this.grid.resizeCanvas();this.grid.focus()};Constr.prototype.update=function(a,b){if(b||a!==this.collection)$.log("Opening resources for %s",a),this.collection=a,this.grid.invalidate(),this.data.length=0,this.grid.setActiveCell(0,
1),this.grid.setSelectedRows([]),this.grid.onViewportChanged.notify(),this.grid.focus()};Constr.prototype.load=function(a,b){var c=this;$.getJSON("modules/collections.xql",{root:this.collection,view:"r",start:a,end:b},function(d){for(var j=a;j<=b;j++)c.grid.invalidateRow(j);if(d&&d.items){c.data.length=d.total;for(j=0;j<d.items.length;j++)c.data[a+j]=d.items[j]}else c.data.length=0;c.grid.updateRowCount();c.grid.render();0==a&&(c.grid.setActiveCell(0,1),c.grid.focus())})};Constr.prototype.hasSelection=
function(){var a=this.grid.getSelectionModel().getSelectedRows();return a&&0<a.length};Constr.prototype.getSelected=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0==a.length)return null;for(var b=[],c=0;c<a.length;c++)b.push(this.collection+"/"+this.data[a[c]].name);return b};Constr.prototype.deleteResource=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0!=a.length){for(var b=[],c=0;c<a.length;c++)b.push(this.data[a[c]].name);var d=this;eXide.util.Dialog.input("Confirm Deletion",
"Are you sure you want to delete the selected resources?",function(){$.log("Deleting resources %o",b);$.getJSON("modules/collections.xql",{remove:b,root:d.collection},function(a){d.reload();"fail"==a.status&&eXide.util.Dialog.warning("Delete Resource Error",a.message)})})}};Constr.prototype.properties=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0!=a.length){for(var b=[],c=0;c<a.length;c++)b.push(this.collection+"/"+this.data[a[c]].name);$("#resource-properties-content").load("modules/collections.xql",
...
eXide.browse.Browser=function(){function a(a,d,e,b,f){var h=document.createElement("button");h.title=d;h.id="eXide-browse-toolbar-"+e;h.tabindex=b;d=document.createElement("img");d.src="resources/images/"+f;h.appendChild(d);a.append(h);return h}Constr=function(c){var d=this;this.mode="open";var e=$(".eXide-browse-toolbar",c),b=a(e,"Reload","reload",1,"arrow_refresh.png");$(b).click(function(){d.resources.reload(!0);d.collections.reload()});this.btnCreateCollection=a(e,"Create Collection","create",
3,"folder_add.png");$(this.btnCreateCollection).click(function(a){a.preventDefault();d.collections.createCollection()});this.btnUpload=a(e,"Upload Files","upload",4,"database_add.png");$(this.btnUpload).click(function(a){a.preventDefault();$(".eXide-browse-resources",c).hide();$(".eXide-browse-upload",c).show()});this.btnDeleteResource=a(e,"Delete","delete-resource",5,"bin.png");$(this.btnDeleteResource).click(function(a){a.preventDefault();d.deleteSelected()});this.btnProperties=a(e,"Properties",
"properties",10,"application_form_edit.png");$(this.btnProperties).click(function(a){a.preventDefault();d.resources.properties()});b=a(e,"Open Selected","open",6,"page_edit.png");$(b).click(function(a){a.preventDefault();eXide.app.openSelectedDocument(!1)});this.btnCopy=a(e,"Copy","copy",7,"page_copy.png");this.btnCut=a(e,"Cut","cut",8,"cut.png");this.btnPaste=a(e,"Paste","paste",9,"page_paste.png");this.selection=$(".eXide-browse-form input",c);this.container=c;this.resources=new eXide.browse.ResourceBrowser($(".eXide-browse-resources",
...
this,this.onChangeCollection);this.upload.addEventListener("done",this,function(){$(".eXide-browse-resources",c).show();$(".eXide-browse-upload",c).hide();this.reload()});$(this.btnCopy).click(function(a){a.preventDefault();d.resources.copy()});$(this.btnCut).click(function(a){a.preventDefault();d.resources.cut()});$(this.btnPaste).click(function(a){a.preventDefault();d.resources.paste()})};Constr.prototype={init:function(){var a=$(this.container).innerHeight()-$(".eXide-browse-form",this.container).height()-
25;$(".eXide-browse-panel",this.container).height(a);null==this.layout&&(this.layout=$(".eXide-browse-panel",this.container).layout({enableCursorHotkey:!1,north__resizable:!1,north__closable:!1,north__spacing_open:0,south__resizable:!1,west__size:200,west__initClosed:!1,west__contentSelector:".eXide-browse-content",center__minSize:300,center__contentSelector:".eXide-browse-content",onresize:function(){this.resources.resize()}}),this.resources.resize())},reload:function(a,d){if(a){$(".eXide-browse-toolbar button",
this.container).hide();for(var e=0;e<a.length;e++)$("#eXide-browse-toolbar-"+a[e]).show()}d&&(this.mode=d);this.resources.setMode(d);"save"===this.mode?$(".eXide-browse-form",this.container).show().focus():$(".eXide-browse-form",this.container).hide();null!=this.layout&&(this.resize(),this.collections.reload(),this.resources.update(this.collections.getSelection(),!0),$(this.selection).val(""))},resize:function(){var a=$(this.container).innerHeight()-$(".eXide-browse-form",this.container).height()-
25;$(".eXide-browse-panel",this.container).height(a);this.layout.resizeAll()},deleteSelected:function(){var a=this.resources.getSelected();if(null==a){var a=this.collections.getSelected(),d=this;eXide.util.Dialog.input("Confirm Deletion","Are you sure you want to delete the selected collections?",function(){$.log("Deleting collection %o",a);$.getJSON("modules/collections.xql",{remove:[a.name]},function(a){d.reload();"fail"==a.status&&eXide.util.Dialog.warning("Delete Collection Error",a.message)})})}else this.resources.deleteResource()},
getSelection:function(){var a=$(this.selection).val();return null==a||""==a?null:{name:a,path:this.collections.getSelection()+"/"+a,writable:!0}},onActivateResource:function(a,d){a?$(this.selection).val(a.name):$(this.selection).val("");"open"!=this.mode&&d?($(this.btnDeleteResource).css("display",""),$(this.btnProperties).css("display","")):($(this.btnDeleteResource).css("display","none"),$(this.btnProperties).css("display","none"))},onActivateCollection:function(a,d){"open"!=this.mode&&d?($(this.btnCreateCollection).css("display",
...
d)},onChangeCollection:function(a){this.collections.select(a);this.collections.reload();this.resources.update(this.collections.getSelection())},onChange:function(){$.log("Reloading db manager views");this.collections.reload()}};return Constr}();eXide.namespace("eXide.find.IncrementalSearch");
eXide.find.IncrementalSearch=function(){var a={backwards:!1,wrap:!0,caseSensitive:!1,wholeWord:!1,regExp:!1};Constr=function(a,d){var e=this;e.input=$(a);e.input.hide();e.input.keydown(function(a){switch(a.which){case 27:case 13:a.preventDefault();e.input.hide();e.editor.focus();break;case 40:a.preventDefault();e.editor.findNext();break;case 38:a.preventDefault();e.editor.findPrevious();break;default:e.onChange()}});e.editor=d};Constr.prototype.start=function(){this.currentLine=this.editor.getSession().getSelection().getSelectionLead().row;
...
eXide.find.Modules=function(){var a={open:[],"import":[]},c=[{id:"prefix",name:"Prefix",field:"prefix",sortable:!0,resizable:!0,maxWidth:100,minWidth:60},{id:"uri",name:"URI",field:"uri",sortable:!0,resizable:!0,minWidth:100},{id:"at",name:"Location",field:"at",sortable:!0,resizable:!0}],d={editable:!1,multiSelect:!1,forceFitColumns:!0},e=[],b;$(document).ready(function(){$.log("Initializing import module dialog");$("#select-module-dialog").dialog({modal:!1,autoOpen:!1,height:400,width:600,open:eXide.find.Modules.$init});
b=new Slick.Grid("#module-list",e,c,d);var a=new Slick.RowSelectionModel({selectActiveRow:!0});b.setSelectionModel(a);b.onDblClick.subscribe(function(a){a=b.getCellFromEvent(a);eXide.find.Modules.$triggerEvent("open",[e[a.row]])});b.onSort.subscribe(function(a,c){$.log("sorting on field: %s",c.sortCol.field);e.sort(function(a,b){var d=c.sortCol.field,f=a[d],d=b[d],f=(f==d?0:f>d?1:-1)*(c.sortAsc?1:-1);return 0!=f?f:0});b.invalidate();b.render()})});return{select:function(a){var b={Open:function(){eXide.find.Modules.trigger("open");
$(this).dialog("close")},Cancel:function(){$(this).dialog("close")}};"xquery"==a&&(b.Import=function(){eXide.find.Modules.trigger("import");$(this).dialog("close")});$("#select-module-dialog").dialog("option","buttons",b);$("#select-module-dialog").dialog("open")},trigger:function(a){var c=b.getSelectionModel().getSelectedRows();1==c.length&&eXide.find.Modules.$triggerEvent(a,[e[c[0]]])},$init:function(){b.resizeCanvas();b.invalidate();e.length=0;$.getJSON("modules/find.xql",function(a){e.length=
a.length;for(var c=0;c<a.length;c++)e[c]=a[c];b.updateRowCount();b.render();b.setActiveCell(0,0);b.setSelectedRows([0]);$("#module-list").find(".grid-canvas").focus()})},addEventListener:function(b,c,d){(b=a[b])&&b.push({obj:c,callback:d})},$triggerEvent:function(b,c){var d=a[b];if(d)for(var e=0;e<d.length;e++)d[e].callback.apply(d[e].obj,c)}}}();eXide.namespace("eXide.util.Menubar");
eXide.util.Menubar=function(){Constr=function(a){var c=this;this.container=a;var d=!1;$("ul li",this.container).click(function(a){a.stopPropagation();$("ul",this).css({display:"block"});d=!0});$("body").click(function(){d&&$("ul li ul",c.container).css({display:"none"})})};Constr.prototype.click=function(a,c,d){var e=this;if(d){var b=eXide.edit.commands.getShortcut(d);b&&(b=b.replace(/Command/g,String.fromCharCode(8984)),b=b.replace(/Shift/g,String.fromCharCode(8679)),b=b.replace(/Option/g,String.fromCharCode(8997)),
b=b.replace(/Control/g,"^"),$(a).each(function(){var a=document.createElement("span");a.className="shortcut";a.appendChild(document.createTextNode(b));this.appendChild(a)}))}$(a).click(function(a){a.preventDefault();$("ul li ul",e.container).css({display:"none"});c()})};Constr.prototype.add=function(a,c,d,e){var b=this,a=$(this.container).find('ul li[title="'+a+'"]'),a=a.find("ul");a.append($('<li><a href="#" title="'+d+'">'+c+"</a></li>"));a.find("li:last-child a").click(function(a){a.preventDefault();
e();$("ul li ul",b.container).css({display:"none"})})};Constr.prototype.remove=function(a,c){$.log("Removing menu entry %s",c);a=$(this.container).find('ul li[title="'+a+'"]');a.find('ul li a[title = "'+c+'"]').remove()};return Constr}();