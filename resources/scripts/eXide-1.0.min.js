...
"\n"+e,selection:[1,d.length,1,d.length]}):!1});this.add("comments","insertion",function(d,a,e,f,b){d=e.getCursorPosition();f=f.doc.getLine(d.row);if("\n"==b&&f.match(/^[\(\s]:/))return{text:"\n : ",selection:[1,3,1,3]};if(":"==b&&"("==f.substring(d.column-1,1))return $.log("cursor: %d",d.column),{text:":  :",selection:[2,2]}});this.add("slash","insertion",function(d,e,f,b,c){"/"==c&&(d=f.getCursorPosition(),e=b.doc.getLine(d.row),0<d.column&&"<"==e.charAt(d.column-1)&&(e=e.substring(0,d.column)+
"/"+e.substring(d.column),f=b.doc.getAllLines(),f[d.row]=e,a.exec("closeTag",f.join(b.doc.getNewLineCharacter()),d.row)));return!1})};c.inherits(e,f);b.XQueryBehaviour=e});
...
0),d=e;d<=f;d++)e=a.getLine(d),g.start.row=d,g.end.row=d,g.end.column=e.length,a.replace(g,b?e.match(c)[1]:"<\!--"+e+"--\>")}}).call(i.prototype);b.Mode=i});var eXide=eXide||{};eXide.namespace=function(a){var a=a.split("."),b=eXide,c;"eXide"==a[0]&&(a=a.slice(1));for(c=0;c<a.length;c++)"undefined"==typeof b[a[c]]&&(b[a[c]]={}),b=b[a[c]];return b};eXide.namespace("eXide.util");
...
eXide.edit.Document=function(){Constr=function(a,b,c){this.name=a;this.path=b;this.mime=null;this.syntax="xquery";this.saved=!1;this.editable=!0;this.functions=[];this.helper=null;this.history=[];this.$session=c;a=eXide.app.getPreference("softWrap");this.$session.setUseWrapMode(0!=a);0<a?this.$session.setWrapLimitRange(a,a):0>a&&this.$session.setWrapLimitRange(null,null)};Constr.prototype.getText=function(){return this.$session.getValue()};Constr.prototype.getName=function(){return this.name};Constr.prototype.getPath=
function(){return this.path};Constr.prototype.setPath=function(a){this.path=a};Constr.prototype.getBasePath=function(){return this.path.replace(/(^.+)\/[^\/]*$/,"$1")};Constr.prototype.getMime=function(){return this.mime};Constr.prototype.getSyntax=function(){return this.syntax};Constr.prototype.setSyntax=function(a){this.syntax=a};Constr.prototype.getSession=function(){return this.$session};Constr.prototype.isSaved=function(){return this.saved};Constr.prototype.isEditable=function(){return this.editable};
Constr.prototype.isXQuery=function(){return"application/xquery"==this.mime};Constr.prototype.setModeHelper=function(a){this.helper=a};Constr.prototype.getModeHelper=function(){return this.helper};Constr.prototype.addToHistory=function(a){this.history.push(a)};Constr.prototype.getLastLine=function(){return this.history.pop(line)};Constr.prototype.getCurrentLine=function(){return this.$session.getSelection().getSelectionLead().row};return Constr}();eXide.namespace("eXide.edit.Editor");
...
Constr.prototype.init=function(){0==this.documents.length&&this.newDocument()};Constr.prototype.exec=function(){if(this.activeDoc.getModeHelper()){var a=Array.prototype.slice.call(arguments,1);this.activeDoc.getModeHelper().exec(arguments[0],this.activeDoc,a)}else eXide.util.message("Not supported in this mode.")};Constr.prototype.getActiveDocument=function(){return this.activeDoc};Constr.prototype.getText=function(){return this.activeDoc.getText()};Constr.prototype.newDocument=function(a,e){$.log("Type: %s",
e);for(var b=0,f=0;f<this.documents.length;f++)this.documents[f].path.match(/^__new__(\d+)/)&&(b=parseInt(RegExp.$1));b++;f=a&&"string"==typeof a?new c(a):e&&"xquery"===e?new c('xquery version "3.0";\n'):new c("");b=new eXide.edit.Document("new-document "+b,"__new__"+b,f);e?b.setSyntax(e):b.setSyntax("text");this.$initDocument(b)};Constr.prototype.newDocumentWithText=function(a,e,b){a=new eXide.edit.Document(b.name,b.path,new c(a));a.editable=b.writable;a.mime=e;a.syntax=eXide.util.mimeTypes.getLangFromMime(e);
a.saved=!1;b.line&&(a.addToHistory(b.line),this.editor.gotoLine(b.line));this.$initDocument(a)};Constr.prototype.openDocument=function(a,e,b){b.writable?eXide.util.message("Opening "+b.path):eXide.util.message("Opening "+b.path+" readonly!");a=new eXide.edit.Document(b.name,b.path,new c(a));a.editable=b.writable;a.mime=e;a.syntax=eXide.util.mimeTypes.getLangFromMime(e);a.saved=!0;b.line&&(a.addToHistory(b.line),e=a.$session.getSelection(),e.clearSelection(),e.moveCursorTo(b.line,1),a.$session.setScrollTop(b.line));
$.log("opening %s, mime: %s, syntax: %s, line: %i",b.name,a.mime,a.syntax,b.line);this.$initDocument(a)};Constr.prototype.$initDocument=function(a){var e=this;e.$setMode(a,!1);a.$session.setUndoManager(new f);a.$session.addEventListener("change",function(){a.saved&&(a.saved=!1,e.updateTabStatus(a.path,a));e.triggerCheck()});e.addTab(a);e.editor.setSession(a.$session);e.editor.resize();e.editor.focus()};Constr.prototype.setMode=function(a){this.activeDoc.syntax=a;this.$setMode(this.activeDoc,!0)};
Constr.prototype.$setMode=function(a,e){switch(a.getSyntax()){case "xquery":var b=require("eXide/mode/xquery").Mode;a.$session.setMode(new b(this));e&&(a.mime="application/xquery");break;case "xml":b=require("eXide/mode/xml").Mode;a.$session.setMode(new b(this));e&&(a.mime="application/xml");break;case "html":b=require("eXide/mode/html").Mode;a.$session.setMode(new b(this));e&&(a.mime="text/html");break;case "javascript":b=require("ace/mode/javascript").Mode;a.$session.setMode(new b);e&&(a.mime="application/x-javascript");
break;case "css":b=require("ace/mode/css").Mode;a.$session.setMode(new b);e&&(a.mime="text/css");break;case "text":b=require("ace/mode/text").Mode,a.$session.setMode(new b),e&&(a.mime="text/text");case "less":b=require("ace/mode/less").Mode,a.$session.setMode(new b),e&&(a.mime="application/less")}a.setModeHelper(this.modes[a.getSyntax()])};Constr.prototype.closeDocument=function(){this.$triggerEvent("close",[this.activeDoc]);$('#tabs a[title="'+this.activeDoc.path+'"]').parent().remove();for(var a=
0;a<this.documents.length;a++)this.documents[a].path==this.activeDoc.path&&this.documents.splice(a,1);0==this.documents.length?this.newDocument():(this.activeDoc=this.documents[this.documents.length-1],$('#tabs a[title="'+this.activeDoc.path+'"]').addClass("active"),this.editor.setSession(this.activeDoc.$session),this.editor.resize(),this.$triggerEvent("activate",[this.activeDoc]))};Constr.prototype.saveDocument=function(a,e,b){var c=this,f=c.activeDoc.path,j=c.activeDoc.name;a&&(c.activeDoc.path=
a.path,c.activeDoc.name=a.name);eXide.util.message("Storing resource "+c.activeDoc.name+"...");a={path:c.activeDoc.path,data:c.activeDoc.getText()};c.activeDoc.mime&&(a.mime=c.activeDoc.mime);$.ajax({url:"modules/store.xql",type:"POST",data:a,dataType:"json",success:function(a){if(a.status=="error")b?b.apply(c.activeDoc,[a.message]):eXide.util.error(a.message);else{c.activeDoc.saved=true;c.updateTabStatus(f,c.activeDoc);e?e.apply(c.activeDoc):eXide.util.message(c.activeDoc.name+" stored.");(a=c.activeDoc.getModeHelper())&&
a.documentSaved&&a.documentSaved(c.activeDoc)}},error:function(a){c.activeDoc.path=f;c.activeDoc.name=j;b?b.apply(c.activeDoc,a.responseText):eXide.util.error(a.responseText)}})};Constr.prototype.reload=function(a){this.activeDoc.getSession().setValue(a)};Constr.prototype.getDocument=function(a){for(var a=eXide.util.normalizePath(a),e=0;e<this.documents.length;e++)if(this.documents[e].path==a)return this.documents[e]};Constr.prototype.onInput=function(a,e){var b=a.getModeHelper();if(b&&b.onInput)b.onInput(a,
e)};Constr.prototype.autocomplete=function(){var a=this.activeDoc.getModeHelper();a&&a.autocomplete&&a.autocomplete(this.activeDoc)};Constr.prototype.getHeight=function(){return $(this.container).height()};Constr.prototype.resize=function(){this.editor.resize()};Constr.prototype.clearErrors=function(){this.editor.getSession().clearAnnotations()};Constr.prototype.forEachDocument=function(a){for(var e=0;e<this.documents.length;e++)a(this.documents[e])};Constr.prototype.addTab=function(a){var e=this,
b="t"+e.tabCounter++,c=a.name;a.saved||(c+="*");$("#tabs li a").removeClass("active");var f=document.createElement("li"),j=document.createElement("a");j.appendChild(document.createTextNode(c));j.className="tab active";j.id=b;j.title=a.path;f.appendChild(j);$("#tabs").append(f);$(j).click(function(b){b.preventDefault();e.switchTo(a)});e.activeDoc=a;e.documents.push(a);e.$triggerEvent("activate",[a]);e.scrollToTab($(j))};Constr.prototype.nextTab=function(){if(!(2>this.documents.length)){for(var a=0,
e=0;e<this.documents.length;e++)if(this.documents[e]==this.activeDoc){a=e;break}a==this.documents.length-1?a=0:a++;this.switchTo(this.documents[a])}};Constr.prototype.previousTab=function(){if(!(2>this.documents.length)){for(var a=0,e=0;e<this.documents.length;e++)if(this.documents[e]==this.activeDoc){a=e;break}0==a?a=this.documents.length-1:a--;this.switchTo(this.documents[a])}};Constr.prototype.switchTo=function(a){this.editor.setSession(a.$session);this.editor.resize();this.activeDoc=a;var e=this;
$("#tabs a").each(function(){var b=$(this);this.title==a.path?(b.addClass("active"),e.scrollToTab(b)):b.removeClass("active")});this.updateStatus("");this.$triggerEvent("activate",[a])};Constr.prototype.updateTabStatus=function(a,e){var b;b=e.saved?e.name:e.name+"*";$('#tabs a[title="'+a+'"]').attr("title",e.path).text(b)};Constr.prototype.scrollToTab=function(a){var b=a.offset().left,a=b+a.outerWidth(),c=$("#tabs").innerWidth(),f=$("#tabs").scrollLeft();a>c?($("#tab-next").show(),$("#tab-prev").show(),
...
a+".writable"]=c.editable?"true":"false",localStorage["eXide."+a+".last-line"]=c.getCurrentLine(),c.saved||(localStorage["eXide."+a+".data"]=c.getText()));a++});localStorage["eXide.documents"]=a};return Constr}();$(document).ready(function(){var a=function(a){if(""==a)return{};for(var c={},f=0;f<a.length;++f){var d=a[f].split("=");2==d.length&&(c[d[0]]=decodeURIComponent(d[1].replace(/\+/g," ")))}return c}(window.location.search.substr(1).split("&"));eXide.app.init(function(b){var c=a.open,f=a.snip;c&&!b[c]?eXide.app.findDocument(a.open):f&&eXide.app.newDocument(f)})});eXide.namespace("eXide.app");
...
$(window).resize(eXide.app.resize);$(window).unload(function(){eXide.app.saveState()});eXide.find.Modules.addEventListener("open",null,function(a){eXide.app.findDocument(a.at)});eXide.find.Modules.addEventListener("import",null,function(b){a.exec("importModule",b.prefix,b.uri,b.at)});e&&e(d)},resize:function(){$("#editor");$(".header");a.resize()},newDocument:function(b,c){a.newDocument(b,c)},findDocument:function(b){var c=a.getDocument(b);null==c?(b={name:b.match(/[^\/]+$/),path:b},eXide.app.$doOpenDocument(b)):
a.switchTo(c)},locate:function(b,c,e){if(null==c)a.exec("locate",b,e);else{$.log("Locating %s in document %s",e,c);var d=a.getDocument(c);null==d?(c={name:c.match(/[^\/]+$/),path:c},eXide.app.$doOpenDocument(c,function(){a.exec("locate",b,e)})):(a.switchTo(d),a.exec("locate",b,e))}},openDocument:function(){c.reload(["reload"],"open");$("#open-dialog").dialog("option","title","Open Document");$("#open-dialog").dialog("option","buttons",{cancel:function(){$(this).dialog("close");a.focus()},open:eXide.app.openSelectedDocument});
$("#open-dialog").dialog("open")},openSelectedDocument:function(a){var b=c.getSelection();b&&eXide.app.$doOpenDocument(b);(void 0==a||a)&&$("#open-dialog").dialog("close")},$doOpenDocument:function(b,c,e){b.path=eXide.util.normalizePath(b.path);var d=a.getDocument(b.path);d&&!e?a.switchTo(d):$.ajax({url:"modules/load.xql?path="+b.path,dataType:"text",success:function(d,f,g){e?a.reload(d):(f=eXide.util.mimeTypes.getMime(g.getResponseHeader("Content-Type")),a.openDocument(d,f,b));c&&c.call(null,b)},
error:function(a){eXide.util.error("Failed to load document "+b.path+": "+a.status+" "+a.statusText)}})},reloadDocument:function(){var b=a.getActiveDocument();b.isSaved()?eXide.app.$reloadDocument(b):eXide.util.Dialog.input("Reload Document","Do you really want to reload the document?",function(){eXide.app.$reloadDocument(b)})},$reloadDocument:function(a){a={name:a.getName(),path:a.getPath()};eXide.app.$doOpenDocument(a,null,!0)},closeDocument:function(){a.getActiveDocument().isSaved()?a.closeDocument():
