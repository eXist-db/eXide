...
"\n"+d,selection:[1,c.length,1,c.length]}):!1});this.add("comments","insertion",function(c,f,a,d,b){c=a.getCursorPosition();d=d.doc.getLine(c.row);if("\n"==b&&d.match(/^[\(\s]:/))return{text:"\n : ",selection:[1,3,1,3]};if(":"==b&&"("==d.substring(c.column-1,1))return $.log("cursor: %d",c.column),{text:":  :",selection:[2,2]}});this.add("slash","insertion",function(c,d,a,b,e){"/"==e&&(c=a.getCursorPosition(),d=b.doc.getLine(c.row),0<c.column&&"<"==d.charAt(c.column-1)&&(d=d.substring(0,c.column)+
"/"+d.substring(c.column),a=b.doc.getAllLines(),a[c.row]=d,f.exec("closeTag",a.join(b.doc.getNewLineCharacter()),c.row)));return!1})};e.inherits(f,d);a.XQueryBehaviour=f});
define("eXide/mode/xquery",function(b,a){var e=b("ace/lib/oop"),d=b("ace/mode/text").Mode,c=b("ace/tokenizer").Tokenizer,f=b("eXide/mode/xquery_highlight_rules").XQueryHighlightRules,g=b("eXide/mode/behaviour/xquery").XQueryBehaviour,h=b("ace/range").Range,i=function(d){this.$tokenizer=new c((new f).getRules());this.$behaviour=new g(d)};e.inherits(i,d);(function(){this.getNextLineIndent=function(c,f,d){c=this.$getIndent(f);f.match(/\s*(?:then|else|return|[{\(]|<\w+>)\s*$/)&&(c+=d);return c};this.checkOutdent=
function(c,f,d){return!/^\s+$/.test(f)?!1:/^\s*[\}\)]/.test(d)};this.autoOutdent=function(c,f,d){c=f.getLine(d).match(/^(\s*[\}\)])/);if(!c)return 0;var c=c[1].length,a=f.findMatchingBracket({row:d,column:c});if(!a||a.row==d)return 0;a=this.$getIndent(f.getLine(a.row));f.replace(new h(d,0,d,c-1),a)};this.$getIndent=function(c){return(c=c.match(/^(\s+)/))?c[1]:""};this.toggleCommentLines=function(c,f,d,a){for(var b=!0,g=/^\s*\(:(.*):\)/,c=d;c<=a;c++)if(!g.test(f.getLine(c))){b=!1;break}for(var e=new h(0,
0,0,0),c=d;c<=a;c++)d=f.getLine(c),e.start.row=c,e.end.row=c,e.end.column=d.length,f.replace(e,b?d.match(g)[1]:"(:"+d+":)")}}).call(i.prototype);a.Mode=i});
define("eXide/mode/behaviour/xml",function(b,a){function e(c,f){var d=!0,a=c.type.split(".");f.split(".").forEach(function(c){if(-1==a.indexOf(c))return d=!1});return d}var d=b("ace/lib/oop"),c=b("ace/mode/behaviour").Behaviour,f=b("ace/mode/behaviour/cstyle").CstyleBehaviour,g=b("ace/token_iterator").TokenIterator,h=function(c){this.inherit(f,["braces","parens","string_dquotes"]);this.parent=c;this.add("brackets","insertion",function(c,f,d,a,b){return"\n"==b&&(f=d.getCursorPosition(),"</"==a.doc.getLine(f.row).substring(f.column,
f.column+2))?(c=this.$getIndent(a.doc.getLine(f.row))+a.getTabString(),a=this.$getIndent(a.doc.getLine(f.row)),{text:"\n"+c+"\n"+a,selection:[1,c.length,1,c.length]}):!1});this.add("slash","insertion",function(f,d,a,b,g){"/"==g&&(f=a.getCursorPosition(),d=b.doc.getLine(f.row),0<f.column&&"<"==d.charAt(f.column-1)&&(d=d.substring(0,f.column)+"/"+d.substring(f.column),a=b.doc.getAllLines(),a[f.row]=d,c.exec("closeTag",a.join(b.doc.getNewLineCharacter()),f.row)));return!1});this.add("autoclosing","insertion",
function(c,f,d,a,b){if(">"==b){c=d.getCursorPosition();d=new g(a,c.row,c.column);a=d.getCurrentToken();f=!1;if(!a||!e(a,"meta.tag")&&(!e(a,"text")||!a.value.match("/"))){do a=d.stepBackward();while(a&&(e(a,"string")||e(a,"keyword.operator")||e(a,"entity.attribute-name")||e(a,"text")))}else f=!0;if(a&&e(a,"meta.tag-name")&&!d.stepBackward().value.match("/"))return d=a.value,f&&(d=d.substring(0,c.column-a.start)),{text:"></"+d+">",selection:[1,1]}}})};d.inherits(h,c);a.XMLBehaviour=h});
define("eXide/mode/xml",function(b,a){var e=b("ace/lib/oop"),d=b("ace/mode/xml").Mode,c=b("ace/tokenizer").Tokenizer,f=b("ace/mode/xml_highlight_rules").XmlHighlightRules,g=b("ace/mode/folding/xml").FoldMode,h=b("eXide/mode/behaviour/xml").XMLBehaviour,i=b("ace/range").Range,j=function(d){this.$tokenizer=new c((new f).getRules());this.$behaviour=new h(d);this.foldingRules=new g};e.inherits(j,d);(function(){this.toggleCommentLines=function(c,f,d,a){for(var b=!0,g=/^\s*<\!--(.*)--\>/,c=d;c<=a;c++)if(!g.test(f.getLine(c))){b=
!1;break}for(var e=new i(0,0,0,0),c=d;c<=a;c++)d=f.getLine(c),e.start.row=c,e.end.row=c,e.end.column=d.length,f.replace(e,b?d.match(g)[1]:"<\!--"+d+"--\>")}}).call(j.prototype);a.Mode=j});
define("eXide/mode/html",function(b,a){var e=b("ace/lib/oop"),d=b("ace/mode/html").Mode,c=b("ace/tokenizer").Tokenizer,f=b("ace/mode/html_highlight_rules").HtmlHighlightRules,g=b("ace/mode/folding/html").FoldMode,h=b("eXide/mode/behaviour/xml").XMLBehaviour,i=b("ace/range").Range,j=b("ace/mode/javascript").Mode,l=b("ace/mode/css").Mode,m=function(d){var a=new f;this.$tokenizer=new c(a.getRules());this.$behaviour=new h(d);this.foldingRules=new g;this.$embeds=a.getEmbeds();this.createModeDelegates({"js-":j,
"css-":l})};e.inherits(m,d);(function(){this.toggleCommentLines=function(c,f,d,a){for(var b=!0,g=/^\s*<\!--(.*)--\>/,c=d;c<=a;c++)if(!g.test(f.getLine(c))){b=!1;break}for(var e=new i(0,0,0,0),c=d;c<=a;c++)d=f.getLine(c),e.start.row=c,e.end.row=c,e.end.column=d.length,f.replace(e,b?d.match(g)[1]:"<\!--"+d+"--\>")}}).call(m.prototype);a.Mode=m});var eXide=eXide||{};eXide.namespace=function(b){var b=b.split("."),a=eXide,e;"eXide"==b[0]&&(b=b.slice(1));for(e=0;e<b.length;e++)"undefined"==typeof a[b[e]]&&(a[b[e]]={}),a=a[b[e]];return a};eXide.namespace("eXide.util");
...
Constr.prototype.loadTheme=function(c,a){if(this.themes[c])return a();require("ace/lib/net");this.themes[c]=1;var b="$shared/resources/scripts/ace/theme-"+c.split("/").pop()+".js",d=document.getElementsByTagName("head")[0],e=document.createElement("script");e.src=b;d.appendChild(e);e.onload=a};Constr.prototype.updateStatus=function(c,a){this.status.innerHTML=c;a&&(this.status.href=a)};Constr.prototype.triggerCheck=function(){if(this.activeDoc.getModeHelper()){var c=this;if(!c.pendingCheck){var a=
(new Date).getTime();c.validateTimeout&&2E3>a-c.lastChangeEvent&&clearTimeout(c.validateTimeout);c.lastChangeEvent=a;c.validateTimeout=setTimeout(function(){c.validate.apply(c)},2E3)}}};Constr.prototype.validate=function(){var c=this;c.$triggerEvent("validate",[c.activeDoc]);var a=c.activeDoc.getModeHelper();a&&a.validate&&(c.pendingCheck=!0,$.log("Running validation..."),a.validate(c.activeDoc,c.getText(),function(){c.pendingCheck=!1}))};Constr.prototype.evalError=function(c){var a=/.*line\s(\d+)/i.exec(c),
b=-1;a&&(b=parseInt(a[1]));$.log("error in line %d",a[1]);this.editor.focus();this.editor.gotoLine(b);a=[{row:b-1,text:c,type:"error"}];this.updateStatus(c);this.editor.getSession().setAnnotations(a)};Constr.prototype.focus=function(){this.editor.focus()};Constr.prototype.saveState=function(){var a=0;$.each(this.documents,function(b,d){if(d.path.match("^__new__.*")){var e=d.getText();e&&0<e.length&&(localStorage["eXide."+a+".path"]=d.path,localStorage["eXide."+a+".name"]=d.name,localStorage["eXide."+
...
north__resizable:!1,north__closable:!0,north__showOverflowOnHover:!0,south__minSize:200,south__initClosed:!0,south__contentSelector:".results",west__size:200,west__initClosed:!1,west__contentSelector:".content",center__minSize:300,center__onresize:eXide.app.resize,center__contentSelector:".content"});$("#open-dialog").dialog({title:"Open file",modal:!1,autoOpen:!1,height:480,width:700,open:function(){e.init()},resize:function(){e.resize()}});$("#login-dialog").dialog({title:"Login",modal:!0,autoOpen:!1,
...
function(){var a=this.grid.getSelectionModel().getSelectedRows();return a&&0<a.length};Constr.prototype.getSelected=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0==a.length)return null;for(var c=[],b=0;b<a.length;b++)c.push(this.collection+"/"+this.data[a[b]].name);return c};Constr.prototype.deleteResource=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0!=a.length){for(var c=[],b=0;b<a.length;b++)c.push(this.data[a[b]].name);var e=this;eXide.util.Dialog.input("Confirm Deletion",
"Are you sure you want to delete the selected resources?",function(){$.log("Deleting resources %o",c);$.getJSON("modules/collections.xql",{remove:c,root:e.collection},function(a){e.reload();"fail"==a.status&&eXide.util.Dialog.warning("Delete Resource Error",a.message)})})}};Constr.prototype.properties=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0!=a.length){for(var c=[],b=0;b<a.length;b++)c.push(this.collection+"/"+this.data[a[b]].name);$("#resource-properties-content").load("modules/collections.xql",
{properties:c});$("#resource-properties-dialog").dialog("open")}};Constr.prototype.cut=function(){this.clipboardMode="move";this.copy()};Constr.prototype.copy=function(){var a=this.grid.getSelectionModel().getSelectedRows();this.clipboard=[];for(var c=0;c<a.length;c++){var b=this.data[a[c]].name;"/"!=b.substr(0,1)&&(b=this.collection+"/"+b);this.clipboard.push(b)}$.log("Clipboard: %o",this.clipboard)};Constr.prototype.paste=function(){var a=this;$.log("Copying resources %o to %s",this.clipboard,this.collection);
var c={root:this.collection};c[this.clipboardMode]=this.clipboard;$.getJSON("modules/collections.xql",c,function(c){$.log(c.status);"fail"==c.status?eXide.util.Dialog.warning("Delete Resource Error",c.message):a.reload()})};Constr.prototype.focus=function(){this.container.find(".grid-canvas").focus()};Constr.prototype.reload=function(){this.update(this.collection,!0)};return Constr}();eXide.namespace("eXide.browse.Upload");
...
3,"folder_add.png");$(this.btnCreateCollection).click(function(a){a.preventDefault();e.collections.createCollection()});this.btnUpload=b(d,"Upload Files","upload",4,"database_add.png");$(this.btnUpload).click(function(c){c.preventDefault();$(".eXide-browse-resources",a).hide();$(".eXide-browse-upload",a).show()});this.btnDeleteResource=b(d,"Delete","delete-resource",5,"bin.png");$(this.btnDeleteResource).click(function(a){a.preventDefault();e.deleteSelected()});this.btnProperties=b(d,"Properties",
